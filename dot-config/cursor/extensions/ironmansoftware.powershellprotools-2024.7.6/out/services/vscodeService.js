...
class VSCodeService {
    ...
    outDataGridView(args, callback) {
        if (!args)
            return;
        const cssUri = vscode.Uri.file(path.join(container_1.Container.context.extensionPath, 'resources', 'datatables.min.css')).with({ scheme: 'vscode-resource' });
        const jsUri = vscode.Uri.file(path.join(container_1.Container.context.extensionPath, 'resources', 'datatables.min.js')).with({ scheme: 'vscode-resource' });
        const panel = vscode.window.createWebviewPanel('poshProToolsDataGrid', args.Title, vscode.ViewColumn.One, {
            enableScripts: true
        });
        var terminal = vscode.window.terminals.find(x => x.name === "PowerShell Extension");
        if (terminal == null) {
    ...
    processMsg(msg, callback) {
        ...
        switch (msg.type) {
            case "Terminated":
                container_1.Container.Log("PowerShell Extension has exited. Will attempt to reconnect.");
