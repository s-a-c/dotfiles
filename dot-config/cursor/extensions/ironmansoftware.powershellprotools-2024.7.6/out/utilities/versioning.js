...
class Versioning {
    constructor() {
        this.DOT_PATTERN = new RegExp("\\.");
        this.NON_DIGIT_PATTERN = new RegExp("\\D");
    }
    compareMajor(version1, version2) {
        return this.compareLevels(version1, version2, 0);
    }
    compareMinor(version1, version2) {
        return this.compareLevels(version1, version2, 1);
    }
    compareRevision(version1, version2) {
        return this.compareLevels(version1, version2, 2);
    }
    compareLevels(version1, version2, index) {
        var stringLength = index + 1, v1 = this.normalize(version1), v2 = this.normalize(version2);
    ...
    ;
    normalize(version) {
        var trimmed = version ? version.replace(/^\s*(\S*(\s+\S+)*)\s*$/, "$1") : '', pieces = trimmed.split(this.DOT_PATTERN), partsLength, parts = [], value, piece, num, i;
    ...
    cmp(x, y) {
        var size = Math.min(x.length, y.length), i;
        for (i = 0; i < size; i += 1) {
            if (x[i] !== y[i]) {
                return x[i] < y[i] ? -1 : 1;
            }
