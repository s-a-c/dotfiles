...
async function activate(context) {
    if (context.globalState.get('ignoreDeprecatedConfig', false) !== true) {
        void (0, utils_1.checkSettings)().then((ret) => {
    ...
    if (vscode.workspace.getConfiguration('r.debugger').get('trackTerminals', false)) {
        (0, terminals_1.trackTerminals)(context.environmentVariableCollection);
    }
    context.subscriptions.push(vscode.commands.registerCommand('r.debugger.updateRPackage', () => (0, installRPackage_1.updateRPackage)(context.extensionPath)), vscode.commands.registerCommand('r.debugger.showDataViewer', (arg) => {
        (0, commands_1.showDataViewer)(arg);
    }));
}
