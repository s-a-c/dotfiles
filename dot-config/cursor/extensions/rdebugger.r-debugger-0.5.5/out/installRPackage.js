...
async function updateRPackage(extensionPath) {
    const url = (0, utils_1.getRDownloadLink)();
    const rPath = (await (0, utils_1.getRStartupArguments)()).path.replace(/^"(.*)"$/, '$1');
    ...
    const args = [
        ...
        '--quiet',
        '-f',
        `${(0, path_1.join)(extensionPath, 'R', 'install.R')}`,
        '--args',
        `${url}`
    ];
    const processExecution = new vscode.ProcessExecution(rPath, args);
    const installationTask = new vscode.Task(taskDefinition, vscode.TaskScope.Global, 'Install vscDebugger', 'R-Debugger', processExecution);
...
function checkPackageVersion(version) {
    const checkLevel = (0, utils_1.config)().get('checkVersion', 'required');
    const rPackageVersions = (0, utils_1.getRequiredRPackageVersion)();
