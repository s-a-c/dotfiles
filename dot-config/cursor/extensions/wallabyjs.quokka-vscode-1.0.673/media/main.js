...
$(function () {
  ...
  $('.feature').each(function () {
    $(this)
      .find('h4')
      .append(
        "<div class='codicon codicon-warning feature-warning' title='This feature is not available because your license has expired.'></div>",
      );
    $(this)
      .find('h4')
      .after(
        "<div class='feature-not-available-links'><a href='' class='renew-url btn btn-link p-0 m-0'>Renew License</a><span class='px-2'> | </span><button type='button' class='request-trial-action btn btn-link p-0 m-0'>Try the feature</button></div>",
      );
    ...
    if ($(this).find('.pro-edition').length) {
      $(this)
        .find('h4')
        .append(
          "<div class='codicon codicon-warning community' title=\"This feature is only available to Quokka 'PRO' users.\"></div>",
        );
      $(this)
        .find('h4')
        .after(
          "<div class='community-links'><a href='' class='purchase-url btn btn-link p-0 m-0'>Purchase Quokka 'PRO'</a><span class='px-2'> | </span><button type='button' class='request-trial-action btn btn-link p-0 m-0'>Try the feature</button></div>",
        );
    }
  });
  ...
  $('.feature').each(function () {
    const [yearFrom, monthFrom, dayFrom] = ($(this).attr('data-display-from') || '2000-01-01')
      .split('-')
      .map((item) => item && parseInt(item, 10));
    const [yearTo, monthTo, dayTo] = ($(this).attr('data-display-to') || '9999-01-01')
      .split('-')
      .map((item) => item && parseInt(item, 10));
    ...
    function processEvent(event) {
      switch (event.command) {
        ...
        case 'activateLicense': {
          activateLicense();
          if (
            licenseDetails.state === 'activated' ||
            licenseDetails.state === 'expired' ||
            licenseDetails.state === 'expiring'
          ) {
