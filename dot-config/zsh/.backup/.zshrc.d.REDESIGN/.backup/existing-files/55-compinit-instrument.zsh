...
if [[ "${ZSH_COMPINIT_INSTRUMENT:-1}" == "0" ]]; then
    zf::debug "# [compinit][instrument] disabled via ZSH_COMPINIT_INSTRUMENT=0"
    return 0
fi
# If we already believe compinit executed, skip
if [[ -n ${ZSH_COMPINIT_RAN:-} || -n ${_COMPINIT_INITIALIZED:-} ]]; then
    zf::debug "# [compinit][instrument] already ran (sentinel detected)"
    return 0
fi
# Ensure compinit is autoloadable
autoload -Uz compinit 2>/dev/null || {
    zf::debug "# [compinit][instrument] compinit not available"
    return 0
}
...
zf::debug "# [compinit][instrument] strategy=$__ci_strategy dump=$ZSH_COMPINIT_DUMPFILE"
# Prepare timing
if typeset -f _zsh_perf_segment_start >/dev/null 2>&1; then
...
if [[ "$__ci_strategy" == "fast" ]]; then
    ...
    if ((insecure_count > 0)); then
        zf::debug "# [compinit][instrument][secure] WARNING: ${insecure_count} insecure path(s) detected"
    fi
    compinit -d "$ZSH_COMPINIT_DUMPFILE" || __ci_rc=$?
    # If failure and fallback enabled, retry fast
    if ((__ci_rc != 0)) && [[ "${ZSH_COMPINIT_FAST_FALLBACK:-1}" == "1" ]]; then
        zf::debug "# [compinit][instrument][secure] retrying fast fallback"
        __ci_strategy="fast"
        compinit -C -d "$ZSH_COMPINIT_DUMPFILE" || __ci_rc=$?
    fi
fi
...
if typeset -f _zsh_perf_segment_end >/dev/null 2>&1; then
    ...
    if [[ -n ${__ci_start_ms:-} && -n ${__ci_end_ms:-} ]]; then
        ...
        zf::debug "# [compinit][instrument][fallback] delta=${_COMPINIT_INSTRUMENT_DELTA_MS}ms"
    fi
fi
if ((__ci_rc == 0)); then
    zf::debug "# [compinit][instrument] completed OK"
else
    zf::debug "# [compinit][instrument] ERROR rc=${__ci_rc}"
fi
# Cleanup temporaries
