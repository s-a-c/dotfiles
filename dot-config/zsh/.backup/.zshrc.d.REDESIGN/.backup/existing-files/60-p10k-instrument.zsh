...
if [[ "${ZSH_P10K_INSTRUMENT:-1}" == "0" ]]; then
    zf::debug "# [p10k][instrument] disabled via ZSH_P10K_INSTRUMENT=0"
    return 0
fi
...
if [[ -z ${ZSH_P10K_FILE:-} || ! -r ${ZSH_P10K_FILE:-/dev/null} ]]; then
    zf::debug "# [p10k][instrument] theme file not found (skipping)"
    return 0
fi

# Abort if previously loaded
if [[ -n ${_P10K_THEME_LOADED:-} ]]; then
    zf::debug "# [p10k][instrument] already marked loaded (skipping)"
    return 0
fi
...
_p10k_fallback_start() {
    ...
    zf::debug "# [p10k][instrument][fallback] start=${P10K_SEGMENT_START_MS}"
}
_p10k_fallback_end() {
    ...
    zf::debug "# [p10k][instrument][fallback] end=${end} delta=${ms}ms"
}
...
source "${ZSH_P10K_FILE}" 2>/dev/null || {
    zf::debug "# [p10k][instrument] ERROR sourcing ${ZSH_P10K_FILE}"
}
...
zf::debug "# [p10k][instrument] loaded file=${ZSH_P10K_FILE}"
