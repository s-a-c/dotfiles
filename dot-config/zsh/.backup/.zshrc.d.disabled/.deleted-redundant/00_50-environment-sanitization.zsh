...
if [[ -z "${ZSH_SOURCE_EXECUTE_LOADED:-}" ]]; then
    ...
    if [[ -f "$detection_script" ]]; then
        ...
        zf::debug "Environment sanitization will work but without context-aware features"
    fi
fi
...
if declare -f context_zf::debug >/dev/null 2>&1; then
    ...
    _sanitize_error() {
        ...
        if declare -f handle_error >/dev/null 2>&1; then
            ...
            zf::debug "ERROR [env_sanitize]: $message"
    ...
    _sanitize_log() {
        zf::debug "# [env_sanitize] $1"
    }
    _sanitize_error() {
        zf::debug "ERROR [env_sanitize]: $1"
        return "${2:-1}"
    }
fi
...
_validate_path_security() {
    ...
    if [[ ${#insecure_paths[@]} -gt 0 ]]; then
        ...
        if [[ "$ZSH_SANITIZATION_STRICT_MODE" == "true" ]]; then
            ...
            export PATH=$(IFS=':'; zf::debug "${secure_paths[*]}")
        fi
...
_sanitize_environment() {
    ...
    if command -v bc >/dev/null 2>&1; then
        sanitization_duration=$(zf::debug "$sanitization_end_time - $sanitization_start_time" | bc 2>/dev/null || zf::debug "0.001")
    else
...
_env_sanitization_main() {
    zf::debug "========================================================"
    zf::debug "ZSH Environment Sanitization Security Module"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_ENV_SANITIZATION_VERSION"
    zf::debug "Loaded: $ZSH_ENV_SANITIZATION_LOADED"
    zf::debug ""
    ...
    zf::debug "Running comprehensive environment sanitization..."
    if _sanitize_environment; then
        zf::debug ""
        zf::debug "🎉 Environment sanitization completed successfully!"
        ...
        zf::debug ""
        zf::debug "❌ Environment sanitization completed with security issues."
        if declare -f safe_exit >/dev/null 2>&1; then
            safe_exit 1
        else
            exit 1
