...
[[ "$ZSH_DEBUG" == "1" ]] && {
    zf::debug "# ++++++ $0 ++++++++++++++++++++++++++++++++++++"
    zf::debug "# [environment] Loading interactive environment configuration"
}
...
_zqs-set-setting() {
    if [[ $# -eq 2 ]]; then
        mkdir -p "$_ZQS_SETTINGS_DIR"
            zf::debug "$2" > "${_ZQS_SETTINGS_DIR}/$1"
    else
            zf::debug "Usage: _zqs-set-setting-value SETTINGNAME VALUE"
    fi
}
...
_zqs-delete-setting(){
    ...
    if [[ -f "$sfile" ]]; then
        rm -f "$sfile"
            ...
            zf::debug "There is no settings file for ${1}"
    fi
}
...
function _zqs-update-stale-settings-files() {
    ...
    if [[ -f $ZDOTDIR/.zqs-additional-plugins ]]; then
        ...
        rm -f $ZDOTDIR/.zqs-additional-plugins
            zf::debug "Plugins from .zqs-additional-plugins were moved to .zshrc.add-plugins.d/0000-transferred-plugins with a format change"
    fi
    if [[ -f $ZDOTDIR/.zsh-quickstart-use-bullet-train ]]; then
        ...
        rm -f $ZDOTDIR/.zsh-quickstart-use-bullet-train
            zf::debug "Converted old $ZDOTDIR/.zsh-quickstart-use-bullet-train to new settings system"
    fi
    if [[ -f $ZDOTDIR/.zsh-quickstart-no-omz ]]; then
        _zqs-set-setting load-omz-plugins false
        rm -f $ZDOTDIR/.zsh-quickstart-no-omz
            zf::debug "Converted old $ZDOTDIR/.zsh-quickstart-no-omz to new settings system"
    fi
    if [[ -f $ZDOTDIR/.zsh-quickstart-no-zmv ]]; then
        _zqs-set-setting no-zmv true
        rm -f $ZDOTDIR/.zsh-quickstart-no-zmv
            zf::debug "Converted old $ZDOTDIR/.zsh-quickstart-no-zmv to new settings system"
    fi
    ...
    if [[ -z "ZSH_QUICKSTART_SKIP_TRAPINT" ]]; then
            zf::debug "'ZSH_QUICKSTART_SKIP_TRAPINT' is deprecated in favor of running 'zqs disable-control-c-decorator' to write a settings knob."
...
function zqs-quickstart-disable-control-c-decorator() {
    _zqs-set-setting control-c-decorator false
        zf::debug "Disabled the control-c decorator in future zsh sessions."
        zf::debug "You can re-enable the quickstart's control-c decorator by running 'zqs enable-control-c-decorator'"
}

function zqs-quickstart-enable-control-c-decorator() {
        zf::debug "The control-c decorator is enabled for future zsh sessions."
        zf::debug "You can disable the quickstart's control-c decorator by running 'zqs disable-control-c-decorator'"
...
function zsh-quickstart-check-for-ssh-askpass() {
    if ! can_haz ssh-askpass; then
            zf::debug "If you disable the ssh-askpass-require feature, you'll"
            zf::debug "need to install ssh-askpass for the quickstart to prompt,"
            zf::debug "for your ssh key/s passphrase on shell startup."
            zf::debug "This is the default behavior for ssh-add:"
            zf::debug $(tput setaf 2)"https://www.man7.org/linux/man-pages/man1/ssh-add.1.html#ENVIRONMENT"$(tput sgr0)
    fi
}
    ...
    if [[ $(_zqs-get-setting use-1password-ssh-agent true) == 'true' ]]; then
        ...
        if [[ -r "$ONE_P_SOCK" ]];then
            export SSH_AUTH_SOCK="$ONE_P_SOCK"
                ...
                zf::debug "Quickstart is set to use 1Password's ssh agent, but $ONE_P_SOCK isn't readable!"
        fi
...
if [[ "$(uname -s)" == "Darwin" ]]; then
    ...
    if [ -d ~/.osx_aliases.d ]; then
            zf::debug "Apple renamed the os to macos - the .osx_aliases.d directory is deprecated in favor of .macos_aliases.d"
...
if [ "$TERM" = "screen" -a ! "$SHOWED_SCREEN_MESSAGE" = "true" ]; then
    ...
    if [ ! -z "$detached_screens" ]; then
            zf::debug "+---------------------------------------+"
            zf::debug "| Detached screens are available:             |"
            zf::debug "$detached_screens"
            zf::debug "+---------------------------------------+"
    fi
fi
...
if [[ -d $ZDOTDIR/.zsh-completions ]]; then
        zf::debug '$ZDOTDIR/.zsh_completions is deprecated in favor of $ZDOTDIR/.zsh_completions.d'
...
# Check for auto-updates only if enabled via ZQS settings
if [[ ! -z "$QUICKSTART_KIT_REFRESH_IN_DAYS" ]] && [[ "$(_zqs-get-setting auto-update-enabled true)" == 'true' ]]; then
    _check-for-zsh-quickstart-update
fi
...
if [[ ! -f $ZDOTDIR/.p10k.zsh ]]; then
        zf::debug "Run p10k configure or edit $ZDOTDIR/.p10k.zsh to configure your prompt"
else
...
if ! can_haz fzf; then
        zf::debug "$?"
        zf::debug "You'll need to install fzf or your history search will be broken."
    echo
    echo
        zf::debug "Install instructions can be found at https://github.com/junegunn/fzf/"
fi

function zqs-help() {
        zf::debug "The zqs command allows you to manipulate your ZSH quickstart."
    echo
        zf::debug "Quickstart action commands:"
        zf::debug "zqs check-for-updates - Update the quickstart kit if it has been longer than $QUICKSTART_KIT_REFRESH_IN_DAYS days since the last update."
        zf::debug "zqs selfupdate - Force an immediate update of the quickstart kit"
        zf::debug "zqs update - Update the quickstart kit and all your plugins"
        zf::debug "zqs update-plugins - Update your plugins"
        zf::debug "zqs cleanup - Cleanup unused plugins after removing them from the list"
        zf::debug ""
        zf::debug "Quickstart settings commands:"

        zf::debug "zqs disable-1password-agent - New sessions will not use 1Password's ssh agent"
        zf::debug "zqs enable-1password-agent - New sessions will use 1Password's ssh agent if present."

        zf::debug "zqs disable-bindkey-handling - Set the quickstart to not touch any bindkey settings. Useful if you're using another plugin to handle it."
        zf::debug "zqs enable-bindkey-handling - Set the quickstart to configure your bindkey settings. This is the default behavior."

        zf::debug "zqs enable-control-c-decorator - Creates a TRAPINT function to display '^C' when you type control-c instead of being silent. Default behavior."
        zf::debug "zqs disable-control-c-decorator - No longer creates a TRAPINT function to display '^C' when you type control-c."

        zf::debug "zqs enable-diff-so-fancy - Load the diff-so-fancy ZSH plugin (defaults to true)"
        zf::debug "zqs disable-diff-so-fancy - Don't load the diff-so-fancy ZSH plugin"

        zf::debug "zqs disable-omz-plugins - Set the quickstart to not load oh-my-zsh plugins if you're using the standard plugin list"
        zf::debug "zqs enable-omz-plugins - Set the quickstart to load oh-my-zsh plugins if you're using the standard plugin list"

        zf::debug "zqs enable-ssh-askpass-require - Set the quickstart to prompt for your ssh passphrase on the command line."
        zf::debug "zqs disable-ssh-askpass-require - Set the quickstart to prompt for your ssh passphrase via a gui program. This is the default behavior"

        zf::debug "zqs disable-ssh-key-listing - Set the quickstart to not display all the loaded ssh keys"
        zf::debug "zqs enable-ssh-key-listing - Set the quickstart to display all the loaded ssh keys. This is the default behavior."

        zf::debug "zqs disable-ssh-key-loading - Set the quickstart to not load your ssh keys. Useful if you're storing them in a yubikey."
        zf::debug "zqs enable-ssh-key-loading - Set the quickstart to load your ssh keys if they aren't already in an ssh agent. This is the default behavior."

        zf::debug "zqs disable-zmv-autoloading - Set the quickstart to not run 'autoload -U zmv'. Useful if you're using another plugin to handle it."
        zf::debug "zqs enable-zmv-autoloading - Set the quickstart to run 'autoload -U zmv'. This is the default behavior."

        zf::debug "zqs delete-setting SETTINGNAME - Remove a zqs setting file"
        zf::debug "zqs get-setting SETTINGNAME [optional default value] - load a zqs setting"
        zf::debug "zqs set-setting SETTINGNAME value - Set an arbitrary zqs setting"
}
...
# Set/Unset settings
        ...
        'disable-1password-agent')
                zf::debug "Disabling 1password ssh-agent. New ZSH sessions will no longer use 1password's ssh agent."
            _zqs-set-setting use-1password-ssh-agent false
            ;;
        'enable-1password-agent')
                zf::debug "Enabling 1password ssh-agent. New ZSH sessions will use 1password's ssh agent."
            _zqs-set-setting use-1password-ssh-agent true
            ;;
        ...
        'disable-diff-so-fancy')
                zf::debug "Disabling diff-so-fancy plugin. New ZSH sessions will no longer use the plugin."
            _zqs-set-setting diff-so-fancy false
            ;;
        'enable-diff-so-fancy')
                zf::debug "Enabling diff-so-fancy plugin. It will be loaded the next time you start a ZSH session."
            _zqs-set-setting diff-so-fancy true
            ;;
        ...
        'disable-zsh-profiling')
            rm -f $ZDOTDIR/.zqs-zprof-enabled
                zf::debug "New ZSH sessions will no longer use profiling."
            ;;
        'enable-zsh-profiling')
            touch $ZDOTDIR/.zqs-zprof-enabled
                zf::debug "New ZSH sessions will use profiling."
            ;;
        *)
            zqs-help
            ;;
