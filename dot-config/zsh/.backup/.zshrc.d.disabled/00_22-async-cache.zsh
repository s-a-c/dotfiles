...
if [[ -z "${ZSH_SOURCE_EXECUTE_LOADED:-}" ]]; then
    ...
    if [[ -f "$detection_script" ]]; then
        ...
        zf::debug "Async cache will work but without context-aware features"
    fi
fi
...
export ZSH_ASYNC_CACHE_LOADED="$(command -v date >/dev/null && date -u '+    %FT%T %Z' || zf::debug 'loaded')"
...
_generate_cache_key() {
    ...
    local file_hash=$(zf::debug "$source_file:$(stat -f "%m" "$source_file" 2>/dev/null || zf::debug "0")" | shasum -a 256 | cut -d' ' -f1)
...
_is_cache_valid() {
    ...
    # Check TTL
    local cache_age=$(( $(date +%s) - $(stat -f "%m" "$cache_file" 2>/dev/null || zf::debug "0") ))
...
_compile_config() {
    ...
    if zcompile "$compiled_file" "$source_file" 2>/dev/null; then
        ...
        if command -v bc >/dev/null 2>&1; then
            compile_duration=$(zf::debug "scale=3; ($compile_end_time - $compile_start_time) * 1000" | bc 2>/dev/null || zf::debug "unknown")
        else
            compile_duration="<1"
        fi
...
# Mark as loaded
zf::debug "ASYNC_PLUGIN_LOADED:$plugin_name:\$(date +%s)" >> "$ZSH_ASYNC_DIR/loaded_plugins.log"
EOF
    ...
    (
        ...
        if [[ $exit_code -eq 0 ]]; then
            zf::debug "ASYNC_SUCCESS:$plugin_name:$(date +%s)" >> "$ZSH_ASYNC_DIR/async_results.log"
        else
            zf::debug "ASYNC_FAILED:$plugin_name:$exit_code:$(date +%s)" >> "$ZSH_ASYNC_DIR/async_results.log"
        fi
    ) &
...
EOF
    ...
    for cache_key in "${(@k)ZSH_CACHE_REGISTRY}"; do
        zf::debug "ZSH_CACHE_REGISTRY[$cache_key]=\"${ZSH_CACHE_REGISTRY[$cache_key]}\"" >> "$ZSH_CACHE_MANIFEST"
    done

    zf::debug "" >> "$ZSH_CACHE_MANIFEST"
    zf::debug "# Cache metadata" >> "$ZSH_CACHE_MANIFEST"
    zf::debug "export ZSH_CACHE_UPDATED=\"$(date -u '+    %FT%T %Z')\"" >> "$ZSH_CACHE_MANIFEST"
}
...
cache-status() {
    zf::debug "========================================================"
    zf::debug "Advanced Caching and Async Loading Status"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_ASYNC_CACHE_VERSION"
    zf::debug "Async Enabled: $ZSH_ENABLE_ASYNC"
    zf::debug "Compilation Enabled: $ZSH_ENABLE_COMPILATION"
    zf::debug "Cache TTL: ${ZSH_CACHE_TTL}s"
    zf::debug ""

    zf::debug "Cache Directories:"
    zf::debug "  Cache Dir: $ZSH_CACHE_DIR"
    zf::debug "  Compiled Dir: $ZSH_COMPILED_DIR"
    zf::debug "  Async Dir: $ZSH_ASYNC_DIR"
    zf::debug ""

    zf::debug "Cache Statistics:"
    if [[ -d "$ZSH_COMPILED_DIR" ]]; then
        local compiled_count=$(find "$ZSH_COMPILED_DIR" -name "*.zwc" -type f | wc -l)
        zf::debug "  Compiled Files: $compiled_count"
    fi

    if [[ -d "$ZSH_ASYNC_DIR" ]]; then
        local async_count=$(find "$ZSH_ASYNC_DIR" -name "*.zsh" -type f | wc -l)
        zf::debug "  Async Scripts: $async_count"
    fi

    zf::debug "  Cache Registry Entries: ${#ZSH_CACHE_REGISTRY[@]}"
    zf::debug "  Active Async Jobs: ${#ZSH_ASYNC_JOBS[@]}"

    if [[ ${#ZSH_ASYNC_JOBS[@]} -gt 0 ]]; then
        zf::debug ""
        zf::debug "Active Async Jobs:"
        for job_key in "${(@k)ZSH_ASYNC_JOBS}"; do
            local job_info="${ZSH_ASYNC_JOBS[$job_key]}"
            local job_pid="${job_info%:*}"
            local job_start="${job_info#*:}"
            local job_age=$(( $(date +%s) - job_start ))
            zf::debug "  - $job_key (PID: $job_pid, Age: ${job_age}s)"
        done
    fi
}

cache-clean() {
    zf::debug "Cleaning cache..."
    _clean_expired_cache
    _update_cache_manifest
    zf::debug "Cache cleaned successfully"
}

cache-rebuild() {
    zf::debug "Rebuilding cache..."
    rm -rf "$ZSH_COMPILED_DIR" "$ZSH_ASYNC_DIR"
    mkdir -p "$ZSH_COMPILED_DIR" "$ZSH_ASYNC_DIR"
    unset ZSH_CACHE_REGISTRY
    typeset -gA ZSH_CACHE_REGISTRY
    _update_cache_manifest
    zf::debug "Cache rebuilt successfully"
}
...
async_main() {
    zf::debug "========================================================"
    zf::debug "ZSH Advanced Caching and Async Loading System"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_ASYNC_CACHE_VERSION"
    zf::debug "Loaded: $ZSH_ASYNC_CACHE_LOADED"
    zf::debug ""
