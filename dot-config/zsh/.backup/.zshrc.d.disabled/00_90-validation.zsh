...
if [[ -z "${ZSH_SOURCE_EXECUTE_LOADED:-}" ]]; then
    ...
    if [[ -f "$detection_script" ]]; then
        ...
        zf::debug "Validation will work but without context-aware features"
    fi
fi
...
if declare -f context_zf::debug >/dev/null 2>&1; then
    ...
    _validation_error() {
        ...
        if declare -f handle_error >/dev/null 2>&1; then
            ...
            zf::debug "ERROR [validation]: $message"
    ...
    _validation_log() {
        zf::debug "# [validation] $1"
    }
    _validation_error() {
        zf::debug "ERROR [validation]: $1"
        return "${2:-1}"
    }
fi
...
_validate_configuration() {
    ...
    if command -v bc >/dev/null 2>&1; then
        validation_duration=$(zf::debug "$validation_end_time - $validation_start_time" | bc 2>/dev/null || zf::debug "0.001")
    else
...
_validation_main() {
    zf::debug "========================================================"
    zf::debug "ZSH Configuration Validation System"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_VALIDATION_VERSION"
    zf::debug "Loaded: $ZSH_VALIDATION_LOADED"
    zf::debug ""
    ...
    zf::debug "Running comprehensive configuration validation..."
    if _validate_configuration; then
        zf::debug ""
        zf::debug "🎉 Configuration validation completed successfully!"
        ...
        zf::debug ""
        zf::debug "❌ Configuration validation completed with issues."
        if declare -f safe_exit >/dev/null 2>&1; then
            safe_exit 1
        else
            exit 1
