...
[[ "$ZSH_DEBUG" == "1" ]] && {
        zf::debug "# ++++++ $0 ++++++++++++++++++++++++++++++++++++"
    zf::debug "# [tool-environments] Configuring development tools after plugins"
}
...
if command -v node >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] Node.js ecosystem configured"
fi
...
if command -v python3 >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] Python development configured"
fi
...
if command -v rustc >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] Rust development configured"
fi
...
if command -v go >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] Go development configured"
fi
...
if command -v docker >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] Docker configured"
fi
...
if command -v kubectl >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] Kubernetes tools configured"
fi
...
if command -v aws >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] AWS CLI configured"
fi

# Google Cloud SDK
if command -v gcloud >/dev/null 2>&1; then
    export CLOUDSDK_PYTHON_SITEPACKAGES=1
    zf::debug "# [tool-environments] Google Cloud SDK configured"
fi

# Azure CLI
if command -v az >/dev/null 2>&1; then
    export AZURE_CORE_OUTPUT=table
    zf::debug "# [tool-environments] Azure CLI configured"
fi
...
if command -v psql >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] PostgreSQL configured"
fi

if command -v mysql >/dev/null 2>&1; then
    export MYSQL_PS1="(\u@\h) [\d]> "
    zf::debug "# [tool-environments] MySQL configured"
fi
...
if command -v nvim >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] Neovim configured"
fi

# VS Code integration
if command -v code >/dev/null 2>&1; then
    export VSCODE_CWD="$PWD"
    zf::debug "# [tool-environments] VS Code configured"
fi

## [tool-environments.build-tools] - Build and development tools
# Make configuration
if command -v make >/dev/null 2>&1; then
    export MAKEFLAGS="-j$(nproc 2>/dev/null || sysctl -n hw.ncpu 2>/dev/null || zf::debug 4)"
fi

# CMake configuration
if command -v cmake >/dev/null 2>&1; then
    export CMAKE_EXPORT_COMPILE_COMMANDS=1
    export CMAKE_BUILD_PARALLEL_LEVEL="$(nproc 2>/dev/null || sysctl -n hw.ncpu 2>/dev/null || zf::debug 4)"
fi
...
if command -v git >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] Git tools configured"
fi
...
case "$(safe_uname -s)" in
    Darwin)
        ...
        zf::debug "# [tool-environments] macOS optimizations applied"
        ;;
    Linux)
        ...
        zf::debug "# [tool-environments] Linux optimizations applied"
        ;;
    MINGW32_NT*|MINGW64_NT*)
        # Windows/Git Bash optimizations
        export MSYS2_PATH_TYPE=inherit
        zf::debug "# [tool-environments] Windows optimizations applied"
        ;;
esac
...
if command -v composer >/dev/null 2>&1; then
    ...
    unset composer_home
    zf::debug "# [tool-environments] Composer configured"
fi
...
if command -v bob >/dev/null 2>&1; then
    ...
    zf::debug "# [tool-environments] Bob (Neovim manager) configured"
fi

# Bun completions and environment
if [[ -s "${XDG_DATA_HOME}/bun/_bun" ]]; then
    source "${XDG_DATA_HOME}/bun/_bun"
    zf::debug "# [tool-environments] Bun completions loaded"
fi

# Docker Desktop completions
if [[ -d "$HOME/.docker/completions" ]]; then
    fpath=("$HOME/.docker/completions" $fpath)
    zf::debug "# [tool-environments] Docker completions added to fpath"
fi
...
if [[ -d "$HOME/google-cloud-sdk" ]]; then
    ...
    zf::debug "# [tool-environments] Google Cloud SDK integrated"
fi

# MCP Environment Setup for AugmentCode
if [[ -f "$HOME/.mcp-environment.sh" ]]; then
    builtin source "$HOME/.mcp-environment.sh"
    zf::debug "# [tool-environments] MCP AugmentCode environment loaded"
fi
...
if command -v awk >/dev/null 2>&1; then
    local new_path
    new_path=$(zf::debug -n "$PATH" | awk -v RS=: '!($0 in a) {a[$0]; printf("%s%s", length(a) > 1 ? ":" : "", $0)}')
    export PATH="$new_path"
fi

zf::debug "# [tool-environments] ✅ All development tools configured and optimized"
