...
if [[ -z "${ZSH_SOURCE_EXECUTE_LOADED:-}" ]]; then
    ...
    if [[ -f "$detection_script" ]]; then
        ...
        zf::debug "Plugin metadata will work but without context-aware features"
    fi
fi
...
if declare -f context_zf::debug >/dev/null 2>&1; then
    ...
    _plugin_log() {
        ...
        local L=${level:u}
        if [[ "$L" == "ERROR" || "$L" == "WARN" || "$ZSH_STARTUP_VERBOSE" == "true" || "$ZSH_DEBUG" == "1" ]]; then
            zf::debug "# [plugin][$L] $msg"
        fi
    }
    _plugin_error() {
        zf::debug "ERROR [plugin]: $1"
        return "${2:-1}"
    }
fi
...
export ZSH_PLUGIN_METADATA_LOADED="$(command -v date >/dev/null && date -u '+    %FT%T %Z' || zf::debug 'loaded')"
...
register_plugin() {
    ...
    if command -v sed >/dev/null 2>&1; then
        deps_json=$(zf::debug "$dependencies" | sed 's/,/", "/g' | sed 's/^/"/' | sed 's/$/"/')
        conflicts_json=$(zf::debug "$conflicts" | sed 's/,/", "/g' | sed 's/^/"/' | sed 's/$/"/')
    else
...
get_plugin_metadata() {
    ...
    if [[ -n "${ZSH_PLUGIN_REGISTRY[$plugin_name]:-}" ]]; then
        zf::debug "${ZSH_PLUGIN_REGISTRY[$plugin_name]}"
        return 0
    else
...
check_plugin_dependencies() {
    ...
    local dependencies=$(zf::debug "$metadata" | grep '"dependencies"' | sed 's/.*"dependencies": \[\(.*\)\].*/\1/' | tr -d '"' | tr ',' ' ')
...
check_plugin_conflicts() {
    ...
    local conflicts=$(zf::debug "$metadata" | grep '"conflicts"' | sed 's/.*"conflicts": \[\(.*\)\].*/\1/' | tr -d '"' | tr ',' ' ')
...
list_plugins() {
    ...
    if [[ -z "${ZSH_PLUGIN_REGISTRY:-}" ]]; then
        zf::debug "No plugins registered"
        return 0
    fi
    ...
    case "$format" in
        ...
        "detailed")
            for plugin in "${(@k)ZSH_PLUGIN_REGISTRY}"; do
                zf::debug "Plugin: $plugin"
                zf::debug "${ZSH_PLUGIN_REGISTRY[$plugin]}" | sed 's/^/  /'
                zf::debug ""
            done
            ;;
        "json")
            ...
            for plugin in "${(@k)ZSH_PLUGIN_REGISTRY}"; do
                ...
                zf::debug "  \"$plugin\": ${ZSH_PLUGIN_REGISTRY[$plugin]}"
            done
            zf::debug "}"
            ;;
    esac
...
_plugin_metadata_main() {
    zf::debug "========================================================"
    zf::debug "ZSH Enhanced Plugin Metadata System"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_PLUGIN_METADATA_VERSION"
    zf::debug "Loaded: $ZSH_PLUGIN_METADATA_LOADED"
    zf::debug ""
    ...
    zf::debug "Plugin Registry: $ZSH_PLUGIN_REGISTRY_DIR"
    zf::debug "Registered Plugins:"
    list_plugins "simple"
