...
_log_plugin_defer_utils() {
    zf::debug "[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] $*" >> "$_log_file"
}
...
_setup_dev_environment_plugins() {
    ...
    zsh-defer -t 5 -c "
        ...
        zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading development environment plugins' >> \"\$_dev_log\"

        if command -v zgenom >/dev/null 2>&1; then
            # Load development utilities that are nice to have but not essential
            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading git-flow completion' >> \"\$_dev_log\"
            zgenom load bobthecow/git-flow-completion >> \"\$_dev_log\"

            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading colorize plugin' >> \"\$_dev_log\"
            zgenom oh-my-zsh plugins/colorize >> \"\$_dev_log\"

            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Development plugins loaded' >> \"\$_dev_log\"
        else
            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] ERROR: zgenom not available' >> \"\$_dev_log\"
        fi
    "
...
_setup_sysadmin_plugins() {
    ...
    zsh-defer -t 7 -c "
        ...
        zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading system administration plugins' >> \"\$_sysadmin_log\"

        if command -v zgenom >/dev/null 2>&1; then
            # Load system administration utilities
            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading systemd plugin' >> \"\$_sysadmin_log\"
            zgenom oh-my-zsh plugins/systemd >> \"\$_sysadmin_log\"

            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading sudo plugin' >> \"\$_sysadmin_log\"
            zgenom oh-my-zsh plugins/sudo >> \"\$_sysadmin_log\"

            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] System administration plugins loaded' >> \"\$_sysadmin_log\"
        else
            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] ERROR: zgenom not available' >> \"\$_sysadmin_log\"
        fi
    "
...
deferred_plugin_status() {
    zf::debug "Deferred Plugin Loading Status"
    zf::debug "=============================="
    zf::debug "Log File: $_log_file"
    zf::debug ""

    zf::debug "Command Wrappers Active:"
    [[ -n "$_DOCKER_ZSH_LOADED" ]] && zf::debug "  ✅ Docker tools loaded" || zf::debug "  ⏳ Docker tools (loads on first 'docker' command)"
    [[ -n "$_1PASSWORD_OP_LOADED" ]] && zf::debug "  ✅ 1Password CLI loaded" || zf::debug "  ⏳ 1Password CLI (loads on first 'op' command)"
    [[ -n "$_RAKE_COMPLETION_LOADED" ]] && zf::debug "  ✅ Rake completion loaded" || zf::debug "  ⏳ Rake completion (loads on first 'rake' command)"
    [[ -n "$_PYTHON_TOOLS_LOADED" ]] && zf::debug "  ✅ Python tools loaded" || zf::debug "  ⏳ Python tools (loads on first 'pip' command)"
    [[ -n "$_NODE_TOOLS_LOADED" ]] && zf::debug "  ✅ Node.js tools loaded" || zf::debug "  ⏳ Node.js tools (loads on first 'npm' command)"

    zf::debug ""
    zf::debug "Recent Activity:"
    if [[ -f "$_log_file" ]]; then
        tail -10 "$_log_file" | sed 's/^/  /' || zf::debug "  (no recent activity)"
    else
        zf::debug "  (log file not found)"
    fi
}
...
[[ "$ZSH_DEBUG" == "1" ]] && {
    zf::debug "# [performance] Deferred plugin utilities initialized"
    zf::debug "# ------ $0 --------------------------------"
}
