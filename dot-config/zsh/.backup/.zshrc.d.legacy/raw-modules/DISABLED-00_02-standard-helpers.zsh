...
# Standard logging function (fallback to zf::debug from .zshenv)
_helpers_log() {
    zf::debug "# [helpers] $1"
}
...
_helpers_error() {
    ...
    if declare -f handle_error >/dev/null 2>&1; then
        ...
        zf::debug "ERROR [helpers]: $message"
...
safe_log() {
    ...
    zf::debug "[$timestamp] $message" >> "$log_file" 2>/dev/null || true
}
...
debug_log() {
    ...
    if [[ "$ZSH_DEBUG" == "1" ]]; then
        ...
        zf::debug "# [$timestamp] [$level] [$context] $message"
    fi
}
...
validate_helpers() {
    ...
    if (( ${#missing_functions[@]} > 0 )); then
        zf::debug "ERROR: Missing helper functions: ${missing_functions[*]}"
        return 1
    fi
...
test_helpers() {
    ...
    zf::debug "=== Helper Library Self-Test Results ==="
    for result in "${test_results[@]}"; do
        zf::debug "$result"
    done

    local failed_count=$(zf::debug "${test_results[*]}" | grep -c "❌")
    return $failed_count
}
...
_standard_helpers_main() {
    zf::debug "========================================================"
    zf::debug "ZSH Standard Helpers Library"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_HELPERS_VERSION"
    zf::debug "Loaded: $ZSH_HELPERS_LOADED"
    zf::debug ""
    ...
    zf::debug "Running self-test..."
    if test_helpers; then
        zf::debug ""
        zf::debug "🎉 All helper functions are working correctly!"
        ...
        zf::debug ""
        zf::debug "❌ Some helper functions failed self-test."
        if declare -f safe_exit >/dev/null 2>&1; then
            safe_exit 1
        else
            exit 1
