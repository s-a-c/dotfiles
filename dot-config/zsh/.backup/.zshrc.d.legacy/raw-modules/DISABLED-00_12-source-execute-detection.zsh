...
is_being_sourced() {
    ...
    if command -v ps >/dev/null 2>&1; then
        ...
        parent_cmd=$(timeout 1 ps -o comm= -p "$PPID" 2>/dev/null || zf::debug "unknown")
        case "$parent_cmd" in
            *zsh*|*bash*|*sh*)
...
_log_detection_event() {
    ...
    if [[ "$ZSH_SOURCE_EXECUTE_DEBUG" == "true" || "$event_type" == "ERROR" ]]; then
        ...
        # Log the event
        zf::debug "[$timestamp] [$event_type] [$$] $message" >> "$log_file" 2>/dev/null || true
    fi
}
...
detection_info() {
    zf::debug "=== Source/Execute Detection Information ==="
    zf::debug "Execution Context: $(get_execution_context)"
    zf::debug "Script Name (\$0): $0"
    zf::debug "Function Stack Depth: ${#funcstack[@]}"
    zf::debug "Script Path (%N): ${(%):-%N}"
    zf::debug "Current PID: $$"
    zf::debug "Parent PID: $PPID"
    zf::debug "Debug Mode: $ZSH_SOURCE_EXECUTE_DEBUG"
    zf::debug "=========================================="
}
...
test_detection() {
    zf::debug "Testing source/execute detection..."

    if is_being_sourced; then
        zf::debug "✓ Detected as SOURCED"
    else
        zf::debug "✓ Detected as EXECUTED"
    fi

    zf::debug "Context: $(get_execution_context)"

    # Test error handling
    zf::debug "Testing error handling..."
    context_zf::debug "This is an info message" "INFO"
    context_zf::debug "This is a warning message" "WARN"

    zf::debug "Detection test completed."
}
...
if is_being_executed && [[ -z "${ZSH_SOURCE_EXECUTE_TESTING:-}" ]]; then
    zf::debug "Source/Execute Detection System - Self Test Mode"
    zf::debug "=============================================="
    test_detection
    detection_info
    zf::debug ""
    zf::debug "To use this system, source this file instead of executing it:"
    zf::debug "  source $0"
    zf::debug ""
    safe_exit 0
fi
