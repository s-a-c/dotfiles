...
if [[ -z "${ZSH_SOURCE_EXECUTE_LOADED:-}" ]]; then
    ...
    if [[ -f "$detection_script" ]]; then
        ...
        zf::debug "Review cycles will work but without context-aware features"
    fi
fi
...
export ZSH_REVIEW_CYCLES_LOADED="$(command -v date >/dev/null && date -u '+    %FT%T %Z' || zf::debug 'loaded')"
...
_calculate_next_review_date() {
    ...
    if command -v date >/dev/null 2>&1; then
        case "$frequency" in
            "weekly")
                next_date=$(date -d "$last_date + 1 week" '+%Y-%m-%d' 2>/dev/null || date -v+1w -j -f "%Y-%m-%d" "$last_date" '+%Y-%m-%d' 2>/dev/null || zf::debug "$last_date")
                ;;
            "monthly")
                next_date=$(date -d "$last_date + 1 month" '+%Y-%m-%d' 2>/dev/null || date -v+1m -j -f "%Y-%m-%d" "$last_date" '+%Y-%m-%d' 2>/dev/null || zf::debug "$last_date")
                ;;
            "quarterly")
                next_date=$(date -d "$last_date + 3 months" '+%Y-%m-%d' 2>/dev/null || date -v+3m -j -f "%Y-%m-%d" "$last_date" '+%Y-%m-%d' 2>/dev/null || zf::debug "$last_date")
                ;;
            "biannual")
                next_date=$(date -d "$last_date + 6 months" '+%Y-%m-%d' 2>/dev/null || date -v+6m -j -f "%Y-%m-%d" "$last_date" '+%Y-%m-%d' 2>/dev/null || zf::debug "$last_date")
                ;;
            "annual")
                next_date=$(date -d "$last_date + 1 year" '+%Y-%m-%d' 2>/dev/null || date -v+1y -j -f "%Y-%m-%d" "$last_date" '+%Y-%m-%d' 2>/dev/null || zf::debug "$last_date")
                ;;
            *)
                next_date=$(date -d "$last_date + 1 month" '+%Y-%m-%d' 2>/dev/null || date -v+1m -j -f "%Y-%m-%d" "$last_date" '+%Y-%m-%d' 2>/dev/null || zf::debug "$last_date")
                ;;
...
_update_review_schedule() {
    local review_type="$1"
    local completion_date="${2:-$(date '+%Y-%m-%d' 2>/dev/null || zf::debug '2025-08-22')}"
    ...
    if [[ -f "$ZSH_REVIEW_SCHEDULE_FILE" ]]; then
        ...
        while IFS=':' read -r type freq last next; do
            if [[ "$type" == "$review_type" ]]; then
                zf::debug "$review_type:$frequency:$completion_date:$next_date" >> "$temp_file"
            elif [[ ! "$type" =~ ^#.*$ && -n "$type" ]]; then
                zf::debug "$type:$freq:$last:$next" >> "$temp_file"
            else
                zf::debug "$type:$freq:$last:$next" >> "$temp_file"
            fi
        done < "$ZSH_REVIEW_SCHEDULE_FILE"
        ...
        if [[ -n "$next_date" ]]; then
            ...
            if [[ "$today" > "$next_date" || "$today" == "$next_date" ]]; then
                ...
                if command -v date >/dev/null 2>&1; then
                    ...
                    local reminder_date=$(date -d "$next_date - $reminder_days days" '+%Y-%m-%d' 2>/dev/null || zf::debug "$next_date")
...
_log_review_reminder() {
    ...
    zf::debug "$timestamp,$review_type,$reminder_type" >> "$ZSH_REVIEW_REMINDERS_FILE"
}
...
review-status() {
    zf::debug "========================================================"
    zf::debug "Review Cycles Status"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_REVIEW_CYCLES_VERSION"
    zf::debug "Review Cycles Enabled: $ZSH_ENABLE_REVIEW_CYCLES"
    zf::debug "Reminder Days: $ZSH_REVIEW_REMINDER_DAYS"
    zf::debug "Quiet Mode: $ZSH_REVIEW_QUIET_MODE"
    zf::debug ""
    ...
    zf::debug "Current Date: $today"
    zf::debug ""

    zf::debug "Review Schedule:"
...
review-complete() {
    ...
    if [[ -z "$review_type" ]]; then
        zf::debug "Usage: review-complete <type> [notes]"
        zf::debug "Available types: ${(@k)ZSH_REVIEW_TYPES}"
        return 1
    fi

    if [[ -z "${ZSH_REVIEW_TYPES[$review_type]}" ]]; then
        zf::debug "Unknown review type: $review_type"
        zf::debug "Available types: ${(@k)ZSH_REVIEW_TYPES}"
        return 1
    fi
    ...
    zf::debug "$timestamp,$review_type,completed,$notes" >> "$ZSH_REVIEW_HISTORY_FILE"
    _log_review_reminder "$review_type" "completed"

    zf::debug "✅ $review_type review marked as completed on $completion_date"
    zf::debug "📅 Next $review_type review due: ${ZSH_REVIEW_NEXT_DATES[$review_type]}"
}
...
review-schedule() {
    ...
    if [[ -z "$review_type" || -z "$date" ]]; then
        zf::debug "Usage: review-schedule <type> <YYYY-MM-DD>"
        zf::debug "Available types: ${(@k)ZSH_REVIEW_TYPES}"
        return 1
    fi
    ...
    if [[ ! "$date" =~ ^[0-9]{4}-[0-9]{2}-[0-9]{2}$ ]]; then
        zf::debug "Invalid date format. Use YYYY-MM-DD"
        return 1
    fi
    ...
    while IFS=':' read -r type freq last next; do
        if [[ "$type" == "$review_type" ]]; then
            zf::debug "$review_type:$frequency:$last_date:$date" >> "$temp_file"
        elif [[ ! "$type" =~ ^#.*$ && -n "$type" ]]; then
            zf::debug "$type:$freq:$last:$next" >> "$temp_file"
        else
            zf::debug "$type:$freq:$last:$next" >> "$temp_file"
    ...
    zf::debug "📅 $review_type review scheduled for $date"
}
...
review_cycles_main() {
    zf::debug "========================================================"
    zf::debug "ZSH Review Cycles System"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_REVIEW_CYCLES_VERSION"
    zf::debug "Loaded: $ZSH_REVIEW_CYCLES_LOADED"
    zf::debug ""
