...
path_validate() {
    ...
    if [[ -n "$PATH_DEBUG" && ${#invalid_paths[@]} -gt 0 ]]; then
        zf::debug "# [path] Invalid PATH entries removed: ${invalid_paths[*]}"
    fi

    # Rebuild PATH with only valid directories
    PATH=$(IFS=':'; zf::debug "${valid_paths[*]}")
    export PATH
}
...
clear_command_cache() {
    ...
    if declare -A _zsh_command_cache >/dev/null 2>&1; then
        ...
        _zsh_command_cache=()
        zf::debug "# [functions] Command cache cleared"
    fi
}
...
time_function() {
    ...
    zf::debug "# [timing] $func took ${duration}s"
    return $?
}
...
zf::debug "# [00-core] Core functions loaded"
