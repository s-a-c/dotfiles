...
if [[ -z "${ZSH_SOURCE_EXECUTE_LOADED:-}" ]]; then
    ...
    if [[ -f "$detection_script" ]]; then
        ...
        zf::debug "Context-aware config will work but without context-aware features"
    fi
fi
...
if declare -f context_zf::debug >/dev/null 2>&1; then
    ...
    _context_error() {
        ...
        if declare -f handle_error >/dev/null 2>&1; then
            ...
            zf::debug "ERROR [context]: $message"
    ...
    _context_log() {
        ...
        local L=${level:u}
        if [[ "$L" == "ERROR" || "$L" == "WARN" || "$ZSH_STARTUP_VERBOSE" == "true" || "$ZSH_DEBUG" == "1" ]]; then
            zf::debug "# [context][$L] $msg"
        fi
    }
    _context_error() {
        zf::debug "ERROR [context]: $1"
        return "${2:-1}"
    }
fi
...
export ZSH_CONTEXT_AWARE_LOADED="$(command -v date >/dev/null && date -u '+    %FT%T %Z' || zf::debug 'loaded')"
    ...
    if source "$config_file"; then
        ...
        if command -v date >/dev/null 2>&1; then
            ...
            if command -v bc >/dev/null 2>&1; then
                load_duration=$(zf::debug "scale=3; ($load_end_time - $load_start_time) * 1000" | bc 2>/dev/null || zf::debug "unknown")
            else
                load_duration="<1"
            fi
        else
        ...
        if command -v env >/dev/null 2>&1 && command -v sort >/dev/null 2>&1; then
            ...
            if command -v comm >/dev/null 2>&1 && command -v wc >/dev/null 2>&1 && [[ -n "$env_before" ]]; then
                env_changes=$(comm -13 <(zf::debug "$env_before") <(zf::debug "$env_after") | wc -l)
            else
                env_changes="unknown"
            fi
        else
...
_apply_context_configuration() {
    ...
    zf::debug "$dir" > "$ZSH_CONTEXT_CURRENT_FILE" 2>/dev/null || true
...
context-status() {
    zf::debug "========================================================"
    zf::debug "Context-Aware Configuration Status"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_CONTEXT_AWARE_VERSION"
    zf::debug "Enabled: $ZSH_CONTEXT_ENABLE"
    zf::debug "Current Directory: $PWD"
    zf::debug "Previous Directory: ${ZSH_CONTEXT_PREVIOUS_DIR:-none}"
    zf::debug ""

    zf::debug "Detected Contexts:"
...
context-reload() {
    ...
    zf::debug "Context configuration reloaded"
}
...
context-create() {
    ...
    if [[ -z "$context_name" ]]; then
        zf::debug "Usage: context-create <context-name>"
        zf::debug "Example: context-create nodejs"
        return 1
    fi
    ...
    if [[ -f "$config_file" ]]; then
        zf::debug "Context configuration already exists: $config_file"
        return 1
    fi
...
zf::debug "Loaded $context_name context configuration"
EOF

    zf::debug "Created context configuration: $config_file"
    zf::debug "Edit the file to customize the configuration for $context_name contexts"
}
...
context_main() {
    zf::debug "========================================================"
    zf::debug "ZSH Context-Aware Dynamic Configuration System"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_CONTEXT_AWARE_VERSION"
    zf::debug "Loaded: $ZSH_CONTEXT_AWARE_LOADED"
    zf::debug ""
