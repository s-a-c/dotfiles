...
if [[ -z "${ZSH_SOURCE_EXECUTE_LOADED:-}" ]]; then
    ...
    if [[ -f "$detection_script" ]]; then
        ...
        zf::debug "Security check will work but without context-aware features"
    fi
fi
...
if declare -f context_zf::debug >/dev/null 2>&1; then
    ...
    _security_error() {
        ...
        if declare -f handle_error >/dev/null 2>&1; then
            ...
            zf::debug "ERROR [security]: $message"
...
_run_security_audit() {
    ...
    if command -v bc >/dev/null 2>&1; then
        audit_duration=$(zf::debug "$audit_end_time - $audit_start_time" | bc 2>/dev/null || zf::debug "0.001")
    else
...
_security_check_main() {
    zf::debug "========================================================"
    zf::debug "ZSH Configuration Security Audit System"
    zf::debug "========================================================"
    zf::debug "Version: $ZSH_SECURITY_CHECK_VERSION"
    zf::debug "Loaded: $ZSH_SECURITY_CHECK_LOADED"
    zf::debug ""
    ...
    zf::debug "Running comprehensive security audit..."
    if _run_security_audit; then
        zf::debug ""
        zf::debug "🎉 Security audit completed successfully!"
        ...
        zf::debug ""
        zf::debug "❌ Security audit completed with issues."
        if declare -f safe_exit >/dev/null 2>&1; then
            safe_exit 1
        else
            exit 1
