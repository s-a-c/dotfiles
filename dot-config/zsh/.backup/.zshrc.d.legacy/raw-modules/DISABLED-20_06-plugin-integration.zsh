...
[[ "$ZSH_DEBUG" == "1" ]] && {
        zf::debug "# ++++++ $0 ++++++++++++++++++++++++++++++++++++"
    zf::debug "# [plugin-integration] Configuring plugins AFTER they are loaded"
}
...
{
    zf::debug "# [plugins.zsh-abbr.configuration]"
    ...
    if command -v abbr > /dev/null 2>&1; then
        zf::debug "# [plugins.zsh-abbr] Plugin loaded and configured successfully"
        ...
        zf::debug "# [plugins.zsh-abbr] Manually loaded plugin"
    else
        # Only show warning in debug mode since plugin may be loaded later
        zf::debug "⚠️  zsh-abbr plugin not found during initialization - may be loaded later"
    fi
}

## [plugins.zsh-alias-tips.configuration] - Configure alias tips after plugin loads
{
    zf::debug "# [plugins.zsh-alias-tips.configuration]"
...
## [plugins.zsh-autosuggestions.configuration] - Configure autosuggestions after plugin loads
{
    ...
    if [[ -n "${widgets[autosuggest-accept]}" ]]; then
        ...
        if [[ "$bindkey_output" != *"autosuggest-accept"* ]]; then
            zf::debug "# [plugins.zsh-autosuggestions] Setting up key bindings"
        ...
        zf::debug "# [plugins.zsh-autosuggestions] Plugin configured successfully"
    else
        zf::debug "# [plugins.zsh-autosuggestions] Widget not found, attempting manual initialization"
...
{
    zf::debug "# [plugins.fast-syntax-highlighting.configuration]"
    ...
    if [[ -n "${_FAST_HIGHLIGHT_VERSION}" ]]; then
        zf::debug "# [plugins.fast-syntax-highlighting] Plugin configured successfully"
    fi
}

## [plugins.fzf-tab.configuration] - Configure fzf-tab after plugin loads
{
    zf::debug "# [plugins.fzf-tab.configuration]"
    ...
    if [[ -n "${fzf_tab_completion_colors}" ]]; then
        zf::debug "# [plugins.fzf-tab] Plugin configured successfully"
    fi
}

## [plugins.oh-my-zsh.configuration] - Configure Oh-My-Zsh plugins after they load
{
    zf::debug "# [plugins.oh-my-zsh.configuration]"
    ...
    if [[ -n "${NVM_DIR:-}" ]] || [[ -f "$HOME/.nvm/nvm.sh" ]] || [[ -f "/usr/local/opt/nvm/nvm.sh" ]] || [[ -f "/opt/homebrew/opt/nvm/nvm.sh" ]]; then
        ...
        if [[ -n "$NPM_CONFIG_PREFIX" ]]; then
            zf::debug "# [plugins.oh-my-zsh] Unsetting NPM_CONFIG_PREFIX (was: $NPM_CONFIG_PREFIX) for NVM compatibility"
            unset NPM_CONFIG_PREFIX
        fi
        zf::debug "# [plugins.oh-my-zsh] NPM_CONFIG_PREFIX kept unset for NVM compatibility"
    else
        # No NVM detected - safe to have NPM_CONFIG_PREFIX set
        if [[ -z "$NPM_CONFIG_PREFIX" ]]; then
            export NPM_CONFIG_PREFIX="$HOME/.local/share/npm"
            zf::debug "# [plugins.oh-my-zsh] NPM_CONFIG_PREFIX set to $NPM_CONFIG_PREFIX (no NVM)"
        fi
    fi

    # Verify Oh-My-Zsh functionality
    if [[ -n "$ZSH" ]]; then
        zf::debug "# [plugins.oh-my-zsh] Plugins configured successfully"
    fi
}

## [plugins.globalias.configuration] - Configure globalias after plugin loads
{
    zf::debug "# [plugins.globalias.configuration]"
    ...
    if [[ -n "${widgets[globalias]}" ]]; then
        zf::debug "# [plugins.globalias] Plugin configured successfully"
    fi
}
## [plugin-health-check] - Verify all critical plugins are loaded and configured
{
    zf::debug "# [plugin-health-check] Verifying plugin configuration"
    ...
    if [[ ${#configured_plugins[@]} -gt 0 ]]; then
        zf::debug "# [plugin-integration] Successfully configured plugins: ${configured_plugins[*]}"
    fi

    if [[ ${#partially_loaded_plugins[@]} -gt 0 ]]; then
        zf::debug "# [plugin-integration] Partially loaded plugins: ${partially_loaded_plugins[*]}"
    fi

    # Only show warnings for actually failed plugins
    if [[ ${#failed_plugins[@]} -gt 0 ]]; then
        zf::debug "⚠️  Plugin configuration issues detected: ${failed_plugins[*]}"
        zf::debug "💡 This may be normal if plugins are lazy-loaded or optional"
    fi
}

zf::debug "# [plugin-integration] Plugin integration and configuration complete"
