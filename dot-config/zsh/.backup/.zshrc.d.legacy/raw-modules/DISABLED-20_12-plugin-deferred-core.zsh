...
_setup_deferred_framework() {
    ...
    _defer_plugin_load() {
        ...
        zsh-defer -t "$delay" -c "
            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] $log_message' >> '$_log_file'
            if command -v zgenom >/dev/null 2>&1; then
                zgenom load '$plugin_name' >> '$_log_file'
                zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Successfully loaded: $plugin_name' >> '$_log_file'
            else
                zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] ERROR: zgenom not available for loading $plugin_name' >> '$_log_file'
            fi
        "
    }
...
_defer_utility_plugins() {
    ...
    zsh-defer -t 1 -c "
        ...
        zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading deferred utility plugins' >> \"\$_defer_log\"

        # Load utility plugins that enhance but don't provide essential functionality
        if command -v zgenom >/dev/null 2>&1; then
            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading JPB utilities' >> \"\$_defer_log\"
            zgenom load unixorn/jpb.zshplugin >> \"\$_defer_log\"

            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading Warhol colorization' >> \"\$_defer_log\"
            zgenom load unixorn/warhol.plugin.zsh >> \"\$_defer_log\"

            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading tumult utilities' >> \"\$_defer_log\"
            zgenom load unixorn/tumult.plugin.zsh >> \"\$_defer_log\"

            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Utility plugins loaded' >> \"\$_defer_log\"
        else
            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] ERROR: zgenom not available' >> \"\$_defer_log\"
        fi
    "
...
_defer_performance_plugins() {
    ...
    zsh-defer -t 3 -c "
        ...
        zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading performance monitoring plugins' >> \"\$_perf_log\"

        if command -v zgenom >/dev/null 2>&1; then
            # Load performance and monitoring utilities
            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Loading zsh-bench for performance testing' >> \"\$_perf_log\"
            zgenom load romkatv/zsh-bench >> \"\$_perf_log\"

            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] Performance plugins loaded' >> \"\$_perf_log\"
        else
            zf::debug '[$(safe_date '+%Y-%m-%d %H:%M:%S UTC')] ERROR: zgenom not available' >> \"\$_perf_log\"
        fi
    "
...
[[ "$ZSH_DEBUG" == "1" ]] && {
    zf::debug "# [performance] Core deferred plugin loading initialized"
    zf::debug "# ------ $0 --------------------------------"
}
