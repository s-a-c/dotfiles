...
if [[ "${PERF_CAPTURE_FAST:-0}" == "1" ]]; then
  ...
  zf::debug "# [plugin-integrity-core][perf-capture-fast] Skipping core plugin integrity verification (PERF_CAPTURE_FAST=1)"
  return 0
fi
...
_log_plugin_security() {
    ...
    local timestamp="$(safe_date -u '+    %FT%T %Z')"
        ...
        zf::debug "[$timestamp] [$level] $message" >> "$ZSH_PLUGIN_SECURITY_LOG" 2>/dev/null || true

    # Also log to stderr for STRICT violations
    if [[ "$level" == "VIOLATION" && "$ZSH_PLUGIN_SECURITY_LEVEL" == "STRICT" ]]; then
            zf::debug "[PLUGIN SECURITY] $message"
    fi
}
...
_verify_plugin_basic() {
    ...
    if [[ "$current_hash" != "$expected_hash" ]]; then
        ...
        if [[ "$ZSH_PLUGIN_SECURITY_LEVEL" == "STRICT" ]]; then
                zf::debug "[PLUGIN SECURITY] BLOCKING plugin $plugin_name due to hash mismatch"
            return 1
        fi
    else
...
plugin_security_status() {
        zf::debug "Plugin Security Status"
        zf::debug "======================"
        zf::debug "Security Level: $ZSH_PLUGIN_SECURITY_LEVEL"
        zf::debug "Registry Dir: $ZSH_PLUGIN_REGISTRY_DIR"
        zf::debug "Cache Dir: $ZSH_PLUGIN_CACHE_DIR"
        zf::debug "Log File: $ZSH_PLUGIN_SECURITY_LOG"
        zf::debug ""

    if [[ -d "$ZSH_PLUGIN_REGISTRY_DIR" ]]; then
        local registry_count=$(find "$ZSH_PLUGIN_REGISTRY_DIR" -name "*.json" | wc -l)
            zf::debug "Registered Plugins: $registry_count"
    else
            zf::debug "Registry Directory: Not found"
    fi

    if [[ -f "$ZSH_PLUGIN_SECURITY_LOG" ]]; then
        local log_lines=$(wc -l < "$ZSH_PLUGIN_SECURITY_LOG" 2>/dev/null || zf::debug 0)
            zf::debug "Log Entries: $log_lines"

        # Show recent violations
        local violations=$(grep "VIOLATION" "$ZSH_PLUGIN_SECURITY_LOG" 2>/dev/null | tail -5)
        if [[ -n "$violations" ]]; then
                zf::debug ""
                zf::debug "Recent Violations:"
                zf::debug "$violations"
        fi
    else
...
[[ "$ZSH_DEBUG" == "1" ]] && {
        zf::debug "# [security] Core plugin integrity verification loaded"
    printf "# ------ %s --------------------------------
" "$0"
}
