...
if [[ "${PERF_CAPTURE_FAST:-0}" == "1" ]]; then
    ...
    zf::debug "# [plugin-integrity-advanced][perf-capture-fast] Skipping advanced integrity verification (PERF_CAPTURE_FAST=1)"
    return 0
fi
...
_get_plugin_hash_advanced() {
    ...
    if command -v openssl >/dev/null 2>&1; then
        find "$plugin_path" -type f \( -name "*.zsh" -o -name "*.sh" \) -print0 2>/dev/null | \
            sort -z | \
            xargs -0 sh -c 'for file; do     zf::debug "$file"; stat -f "%m" "$file" 2>/dev/null || stat -c "%Y" "$file" 2>/dev/null || zf::debug "0"; cat "$file"; done' -- | \
            openssl dgst -sha256 2>/dev/null | \
            awk '{print $2}' || zf::debug "hash_failed"
    else
        # Fallback to basic hash
        find "$plugin_path" -type f -name "*.zsh" -exec cat {} \; 2>/dev/null | \
            shasum -a 256 2>/dev/null | cut -d' ' -f1 || zf::debug "hash_failed"
    fi
}
...
plugin_security_status_advanced() {
        zf::debug "Advanced Plugin Security Status"
        zf::debug "==============================="
    plugin_security_status  # Call basic status first
        zf::debug ""

    # Check for security tools
        zf::debug "Security Tools:"
    if command -v openssl >/dev/null 2>&1; then
            zf::debug "  ✅ OpenSSL available"
    else
            zf::debug "  ❌ OpenSSL not available (hash verification limited)"
    fi

    if command -v jq >/dev/null 2>&1; then
            zf::debug "  ✅ jq available"
    else
            zf::debug "  ❌ jq not available (registry parsing limited)"
    fi
    ...
    if [[ -f "$registry_file" ]] && command -v jq >/dev/null 2>&1; then
            zf::debug ""
            zf::debug "Trusted Plugins:"
        jq -r '.plugins[] | "  \(.name) (\(.version))"' "$registry_file" 2>/dev/null || zf::debug "  (parsing failed)"
    fi

    # Show recent activity
    if [[ -f "$ZSH_PLUGIN_SECURITY_LOG" ]]; then
            zf::debug ""
            zf::debug "Recent Security Events:"
        tail -10 "$ZSH_PLUGIN_SECURITY_LOG" 2>/dev/null | sed 's/^/  /' || zf::debug "  (no recent events)"
    fi
}
...
plugin_security_update_registry() {
    ...
    if [[ -z "$plugin_name" ]] || [[ -z "$plugin_path" ]]; then
            zf::debug "Usage: plugin_security_update_registry <plugin_name> <plugin_path>"
        return 1
    fi

    if [[ ! -d "$plugin_path" ]]; then
            zf::debug "❌ Plugin path does not exist: $plugin_path"
        return 1
    fi
    ...
    if [[ "$current_hash" == "hash_failed" ]]; then
            zf::debug "❌ Failed to calculate hash for plugin: $plugin_name"
        return 1
    fi
    ...
    _create_plugin_registry_entry "$plugin_name" "$plugin_path"
        ...
        zf::debug "✅ Updated registry entry for plugin: $plugin_name"
        zf::debug "   Hash: $current_hash"

    _log_plugin_security "INFO" "Registry updated for plugin: $plugin_name (hash: $current_hash)"
}

plugin_security_scan_all() {
        zf::debug "Scanning all plugins for integrity issues..."

    if [[ ! -d "${ZGEN_DIR:-$HOME/.zgenom}" ]]; then
            zf::debug "❌ zgenom directory not found"
        return 1
    fi
    ...
    if [[ $issues_found -eq 0 ]]; then
            zf::debug "✅ No integrity issues found"
    else
            zf::debug "⚠️  Found $issues_found plugins with issues"
    fi
}
...
[[ "$ZSH_DEBUG" == "1" ]] && {
        zf::debug "# [security] Advanced plugin integrity verification loaded"
    printf "# ------ %s --------------------------------\n" "$0"
}
