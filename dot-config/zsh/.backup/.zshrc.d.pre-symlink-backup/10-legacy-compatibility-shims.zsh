...
# Ensure zf::debug is available (should already be defined in .zshenv)
typeset -f zf::debug >/dev/null 2>&1 || function zf::debug() {
    emulate -L zsh
    echo "$@"
...
if [[ "${ZSH_DEBUG:-0}" == "1" ]]; then
    ...
    zf::debug "# [legacy-compat] Legacy compatibility loaded (${#zf_funcs[@]} zf:: functions)"
    zf::debug "# [legacy-compat] Core functions: zf::log, zf::warn, zf::debug, zf::ensure_cmd, zf::require, zf::with_timing, zf::timed, zf::list_functions"
    zf::debug "# [legacy-compat] PATH functions: zf::path_prepend, zf::path_append, zf::path_contains"
    zf::debug "# [legacy-compat] SSH helpers: ssh_agent_detect, ssh_agent_start, ssh_agent_ensure, ssh_add_keys_safe"
    if [[ -n "${_LOADED_10_CORE_FUNCTIONS:-}" ]]; then
        zf::debug "# [legacy-compat] Redesign module was loaded successfully"
    else
        zf::debug "# [legacy-compat] Using fallback compatibility functions"
    fi
fi
