...
init_prompt_system() {
    ...
    # Check for available prompt systems and initialize
    if command -v starship >/dev/null 2>&1; then
        _ui_debug "Starship detected - will initialize later in external integrations module"
        # Starship initialization moved to 09-external-integrations.zsh for proper timing
        return 0
    elif command -v powerline-go >/dev/null 2>&1; then
        _ui_debug "Powerline-go prompt detected"
        # Add powerline-go initialization here if needed
    else
...
register_custom_widgets() {
    _ui_debug "Registering custom ZLE widgets..."
    ...
    if [[ -n "${ZLE_VERSION:-}" ]] && zle -l >/dev/null 2>&1; then
        ...
        _ui_debug "✅ Custom ZLE widgets registered successfully"
    else
        _ui_debug "⚠️  ZLE not ready - deferring widget registration"
