...
safe_command() {
    ...
    for location in "${locations[@]}"; do
        ...
        zf::debug "Error: Command '$cmd' not found in PATH or standard locations"
    return 1
}
...
debug_path_state() {
    [[ "$ZSH_DEBUG" != "1" ]] && return 0
        ...
        zf::debug "[PATH DEBUG] Current PATH: $PATH"
        zf::debug "[PATH DEBUG] date location: $(command -v date 2>/dev/null || zf::debug 'NOT FOUND')"
        zf::debug "[PATH DEBUG] mkdir location: $(command -v mkdir 2>/dev/null || zf::debug 'NOT FOUND')"
}

# Log initialization
zf::debug "[PATH-GUARANTEE] Safe command wrappers initialized"
