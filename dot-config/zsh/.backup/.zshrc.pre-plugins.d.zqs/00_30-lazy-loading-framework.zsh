...
zf::debug "# ++++++ $0 ++++++++++++++++++++++++++++++++++++"
zf::debug "# [performance] Loading lazy loading framework"
...
_lazy_log() {
    ...
    local timestamp="$(safe_date -u '+    %FT%T %Z')"
        ...
        zf::debug "[$timestamp] [$level] $message" >> "$ZSH_LAZY_LOADING_LOG" 2>/dev/null || true
}
...
lazy_loading_status() {
    zf::debug "Lazy Loading Framework Status"
    zf::debug "============================="
    zf::debug "Enabled: $ZSH_LAZY_LOADING_ENABLED"
    zf::debug "Timeout: $ZSH_LAZY_LOADING_TIMEOUT seconds"
    zf::debug "Log File: $ZSH_LAZY_LOADING_LOG"
    zf::debug ""

    zf::debug "Lazy Wrappers Active:"
    ...
    for func in "${lazy_functions[@]}"; do
        if command -v "$func" >/dev/null 2>&1; then
            ...
            if functions "$func" | grep -q "_lazy_log.*TRIGGER"; then
                    zf::debug "  ⏳ $func (not yet loaded)"
            else
                    zf::debug "  ✅ $func (loaded)"
            fi
        else
                zf::debug "  ❌ $func (not available)"
        fi
    done
    ...
    if [[ -f "$ZSH_LAZY_LOADING_LOG" ]]; then
        tail -10 "$ZSH_LAZY_LOADING_LOG" 2>/dev/null | sed 's/^/  /' || zf::debug "  (no recent activity)"
    else
...
lazy_loading_force_all() {
        zf::debug "Force loading all lazy components..."
    ...
    zf::debug "✅ Force loading complete"
}
...
zf::debug "# [performance] Lazy loading framework loaded"
zf::debug "# ------ $0 --------------------------------"
