    ...
    _lazy_direnv_bootstrap() {
        ...
        _init_direnv_hook() {
            ...
            if [[ -n "$log_file" && "$ZSH_DEBUG" == "1" && -d "$log_dir" && -w "$log_dir" ]]; then
                ...
                if [[ "$log_file" != "" && "$log_file" != "/" && "$log_file" =~ .*/.* ]]; then
                    {
                        zf::debug "=============================================================================="
                        zf::debug "Lazy Direnv Hook Initialization"
                        ...
                        if eval "$(command direnv hook zsh)" 2>/dev/null; then
                            _DIRENV_HOOKED=1
                            zf::debug "✅ direnv hook initialized successfully"
                        else
                            zf::debug "❌ direnv hook initialization failed"
                            return 1
                        fi
                        zf::debug ""
                        zf::debug "=============================================================================="
                        zf::debug "Lazy direnv hook initialization completed at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
                        zf::debug "=============================================================================="
                    } 2>&1 | tee -a "$log_file"
                else
                ...
                if [[ -n "$log_file" ]]; then
                    {
                        zf::debug "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] initializing direnv hook"
                    } >>"$log_file" 2>&1
                fi
                if eval "$(command direnv hook zsh)" 2>/dev/null; then
                    _DIRENV_HOOKED=1
        ...
        _lazy_direnv_chpwd() {
            if [[ -f ".envrc" && $_DIRENV_HOOKED -eq 0 ]]; then
                zf::debug "# Lazy loading direnv due to .envrc presence"
                _init_direnv_hook
            fi
        }
        ...
        zf::debug "# [lazy-direnv] Lazy direnv wrapper initialized"
    }
    _lazy_direnv_bootstrap
else
    zf::debug "# [lazy-direnv] direnv not found, skipping"
fi
