...
load-our-ssh-keys() {
    ...
    if [ $($key_manager -l | grep -c "The agent has no identities." ) -eq 1 ]; then
        ...
        do
            # Add timeout protection for ssh-add -l to prevent hangs
            local ssh_keys_list=""
            if command -v timeout >/dev/null 2>&1; then
                ssh_keys_list=$(timeout 1 ssh-add -l 2>/dev/null)
            else
                ssh_keys_list=$(ssh-add -l 2>/dev/null)
            fi
            if [ -f ${key} -a -n "$ssh_keys_list" -a $(echo "$ssh_keys_list" | grep -F -c "$(ssh-keygen -l -f $key | awk '{print $2}')" ) -eq 0 ]; then
                $key_manager -q ${key}
            fi
        done
    fi
...
# Also respect NO_ZGENOM and SKIP_PLUGINS flags for debugging
...
# Load safe setup if present (only if plugins are enabled)
if [[ "${NO_ZGENOM:-0}" != "1" ]] && [[ "${SKIP_PLUGINS:-0}" != "1" ]] && [[ -f "$ZDOTDIR/.zgen-setup" ]]; then
...
if [[ $(_zqs-get-setting list-ssh-keys true) == 'true' ]]; then
    ...
    if command -v timeout >/dev/null 2>&1; then
        timeout 1 ssh-add -l 2>/dev/null || echo "  (ssh-agent not responding)"
    else
...
if [[ ! -z "$QUICKSTART_KIT_REFRESH_IN_DAYS" ]]; then
    ...
    if declare -f _check-for-zsh-quickstart-update >/dev/null 2>&1; then
        _check-for-zsh-quickstart-update
    else
        # Function not defined - skip update check
        zsh_debug_echo "# [update] _check-for-zsh-quickstart-update function not found, skipping"
    fi
fi
...
# External tool additions have been moved to:
