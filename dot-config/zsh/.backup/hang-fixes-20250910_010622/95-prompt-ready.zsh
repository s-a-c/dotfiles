...
__pr__capture_prompt_ready() {
    ...
    zf::debug "# [prompt-ready][perf] PROMPT_READY_MS=$PROMPT_READY_MS delta_ref=$delta_ref delta=${PROMPT_READY_DELTA_MS:-n/a} from_start=${PROMPT_READY_FROM_START_MS:-n/a}"
    ...
    if [[ "${PERF_PROMPT_HARNESS:-0}" == "1" ]]; then
        zf::debug "# [prompt-ready][harness] exiting shell after prompt readiness capture"
...
zf::debug "# [prompt-ready] instrumentation installed method=${__pr__install_method}"
...
if [[ "${PERF_FORCE_PRECMD:-0}" == "1" && -z ${PROMPT_READY_MS:-} ]]; then
  {
    ...
    if [[ -z ${PROMPT_READY_MS:-} ]]; then
      zf::debug "# [prompt-ready][force] forcing early capture (no prompt yet)"
      __pr__capture_prompt_ready
    fi
  } &!
fi
...
if [[ "${PERF_PROMPT_DEFERRED_CHECK:-1}" == "1" && -z ${PROMPT_READY_MS:-} ]]; then
  {
    ...
    if [[ -z ${PROMPT_READY_MS:-} ]]; then
      zf::debug "# [prompt-ready][deferred] readiness still missing; capturing approximate"
      __pr__capture_prompt_ready
    fi
  } &!
fi
...
if [[ -n ${ZSH_EXECUTION_STRING:-} && -z ${PROMPT_READY_MS:-} ]]; then
    ...
    zf::debug "# [prompt-ready][perf][fallback] PROMPT_READY_MS=${PROMPT_READY_MS} delta=${PROMPT_READY_DELTA_MS:-n/a}"
fi
