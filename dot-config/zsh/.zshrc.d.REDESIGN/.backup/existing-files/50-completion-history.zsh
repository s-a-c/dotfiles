...
if [[ -z ${_HISTORY_CONFIGURED:-} ]]; then
    ...
    zf::debug "# [post-plugin][history] configured with HISTFILE=$HISTFILE"
fi
...
if [[ -z ${_COMPINIT_DONE:-} ]]; then
    ...
    if command -v compinit >/dev/null 2>&1; then
        ...
        if [[ ! -f "$compdump_file" || $age_hours -gt 24 ]]; then
            ...
            zf::debug "# [post-plugin][completion] rebuilt stale cache (age: ${age_hours}h)"
        else
            compinit -C -d "$compdump_file" 2>/dev/null || true
            zf::debug "# [post-plugin][completion] using fresh cache (age: ${age_hours}h)"
        fi
...
if [[ -n ${PERF_SEGMENT_LOG:-} && -n ${POST_SEG_50_COMPLETION_HISTORY_START_MS:-} && -z ${POST_SEG_50_COMPLETION_HISTORY_MS:-} ]]; then
    ...
    if [[ -n $__post_seg_50_end_ms ]]; then
        ...
        zf::debug "# [post-plugin][perf] segment=50-completion-history delta=${POST_SEG_50_COMPLETION_HISTORY_MS}ms"
    fi
