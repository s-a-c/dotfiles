...
zf::debug "# [post-plugin-boundary] end=${POST_PLUGIN_END_MS:-n/a} total=${POST_PLUGIN_TOTAL_MS:-n/a}"
...
if [[ "${POST_PLUGIN_BOUNDARY_PROMPT_CAPTURE}" == "1" && "${PERF_PROMPT_HARNESS:-0}" == "1" && -n ${PERF_SEGMENT_LOG:-} ]]; then
    if ! grep -q 'PROMPT_READY_COMPLETE' "${PERF_SEGMENT_LOG}" 2>/dev/null; then
        ...
        if [[ -n $_ppb_pr_base ]]; then
            ...
            zf::debug "# [post-plugin-boundary][prompt-immediate] emitted prompt_ready delta=${_ppb_pr_delta}"
        else
            zf::debug "# [post-plugin-boundary][prompt-immediate] skipped (no base ms available)"
        fi
    fi
fi
