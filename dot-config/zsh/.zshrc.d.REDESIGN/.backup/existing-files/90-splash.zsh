...
if [[ -z ${POST_PLUGIN_END_MS:-} ]]; then
    ...
    if [[ -n ${PRE_PLUGIN_END_MS:-} && -n ${POST_PLUGIN_END_MS:-} ]]; then
        ...
        zf::debug "# [post-plugin][perf] POST_PLUGIN_END_REALTIME=$POST_PLUGIN_END_REALTIME ms=${POST_PLUGIN_END_MS} total=${POST_PLUGIN_TOTAL_MS}"
        ...
        zf::debug "# [post-plugin][perf] POST_PLUGIN_END_REALTIME=$POST_PLUGIN_END_REALTIME (missing PRE_PLUGIN_END_MS for delta)"
    fi
fi

# (Optional future splash output; currently no-op when disabled)
if [[ "${ZSH_SPLASH_DISABLE:-0}" == "1" ]]; then
    zf::debug "# [splash] disabled"
else
    zf::debug "# [splash] module loaded (no visual output by design)"
fi
