...
if [[ "${ZSH_GITSTATUS_INSTRUMENT:-1}" == "0" ]]; then
  zsh_debug_echo "# [gitstatus][instrument] disabled via ZSH_GITSTATUS_INSTRUMENT=0"
  return 0
fi

# If already initialized (theme or earlier module), skip
if [[ -n ${_GITSTATUS_INIT_DONE:-} || -n ${GITSTATUS_DAEMON_PID:-} ]]; then
  zsh_debug_echo "# [gitstatus][instrument] already initialized (sentinel present)"
  return 0
fi
...
if [[ -z ${__gs_plugin} ]]; then
  zsh_debug_echo "# [gitstatus][instrument] plugin file not found (skipping)"
  return 0
fi

zsh_debug_echo "# [gitstatus][instrument] using plugin=${__gs_plugin}"
...
if ! source "${__gs_plugin}" 2>/dev/null; then
  zsh_debug_echo "# [gitstatus][instrument] ERROR: failed sourcing ${__gs_plugin}"
fi
...
if [[ "$__wait_ms" =~ ^[0-9]+$ ]] && (( __wait_ms > 0 )); then
  ...
  zsh_debug_echo "# [gitstatus][instrument] readiness wait loops=${__poll_i}/${__max_polls} pid=${GITSTATUS_DAEMON_PID:-none}"
fi
...
if typeset -f _zsh_perf_segment_end >/dev/null 2>&1; then
  ...
  if [[ -n ${__gs_start_ms:-} && -n ${__gs_end_ms:-} ]]; then
    ...
    zsh_debug_echo "# [gitstatus][instrument][fallback] delta=${__gs_delta}ms"
    unset sample
  fi
fi

if [[ "${ZSH_GITSTATUS_VERBOSE:-0}" == "1" ]]; then
  zsh_debug_echo "# [gitstatus][instrument] daemon_pid=${GITSTATUS_DAEMON_PID:-unset}"
fi
