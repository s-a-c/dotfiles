...
function _zqs-update-stale-settings-files() {
    ...
    if [[ -f $ZDOTDIR/.zsh-quickstart-use-bullet-train ]]; then
        ...
        rm -f $ZDOTDIR/.zsh-quickstart-use-bullet-train
            zsh_debug_echo "Converted old $ZDOTDIR/.zsh-quickstart-use-bullet-train to new settings system"
    fi
    if [[ -f $ZDOTDIR/.zsh-quickstart-no-omz ]]; then
        _zqs-set-setting load-omz-plugins false
        rm -f $ZDOTDIR/.zsh-quickstart-no-omz
            zsh_debug_echo "Converted old $ZDOTDIR/.zsh-quickstart-no-omz to new settings system"
    fi
    if [[ -f $ZDOTDIR/.zsh-quickstart-no-zmv ]]; then
        _zqs-set-setting no-zmv true
        rm -f $ZDOTDIR/.zsh-quickstart-no-zmv
            zsh_debug_echo "Converted old $ZDOTDIR/.zsh-quickstart-no-zmv to new settings system"
    fi
    ...
    if [[ -z "ZSH_QUICKSTART_SKIP_TRAPINT" ]]; then
            zsh_debug_echo "'ZSH_QUICKSTART_SKIP_TRAPINT' is deprecated in favor of running 'zqs disable-control-c-decorator' to write a settings knob."
...
if [[ "$(uname -s)" == "Darwin" ]]; then
    ...
    if [ -d ~/.osx_aliases.d ]; then
            zsh_debug_echo "Apple renamed the os to macos - the .osx_aliases.d directory is deprecated in favor of .macos_aliases.d"
...
if [ "$TERM" = "screen" -a ! "$SHOWED_SCREEN_MESSAGE" = "true" ]; then
    ...
    if [ ! -z "$detached_screens" ]; then
            zsh_debug_echo "+---------------------------------------+"
            zsh_debug_echo "| Detached screens are available:             |"
            zsh_debug_echo "$detached_screens"
            zsh_debug_echo "+---------------------------------------+"
    fi
fi
