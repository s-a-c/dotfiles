...
if [[ -z "${ZSH_SOURCE_EXECUTE_LOADED:-}" ]]; then
    ...
    if [[ -f "$detection_script" ]]; then
        ...
        zsh_debug_echo "Completion management will work but without context-aware features"
    fi
fi
...
rebuild-completions() {
    zsh_debug_echo "🔄 Rebuilding ZSH completions..."
    ...
    zsh_debug_echo "✅ Completions rebuilt successfully"
    zsh_debug_echo "📁 Completion dump: $ZSH_COMPDUMP_FILE"
}

# 3.2. Show completion status
completion-status() {
    zsh_debug_echo "========================================================"
    zsh_debug_echo "ZSH Completion Management Status"
    zsh_debug_echo "========================================================"
    zsh_debug_echo "Version: $ZSH_COMPLETION_MANAGEMENT_VERSION"
    zsh_debug_echo "Management Enabled: $ZSH_ENABLE_COMPLETION_MANAGEMENT"
    zsh_debug_echo "Rebuild Threshold: ${ZSH_COMPLETION_REBUILD_DAYS} days"
    zsh_debug_echo "Cleanup Old Files: $ZSH_COMPLETION_CLEANUP_OLD"
    zsh_debug_echo ""

    zsh_debug_echo "Completion Files:"
    zsh_debug_echo "  Dump File: $ZSH_COMPDUMP_FILE"
    if [[ -f "$ZSH_COMPDUMP_FILE" ]]; then
        local file_age
        if command -v stat >/dev/null 2>&1; then
            file_age=$(stat -f "%Sm" -t "%Y-%m-%d %H:%M:%S" "$ZSH_COMPDUMP_FILE" 2>/dev/null || zsh_debug_echo "unknown")
        else
            file_age="unknown"
        fi
        zsh_debug_echo "    Status: ✅ Exists (modified: $file_age)"

        if [[ -f "${ZSH_COMPDUMP_FILE}.zwc" ]]; then
            zsh_debug_echo "    Compiled: ✅ Yes"
        else
            zsh_debug_echo "    Compiled: ❌ No"
        fi
    else
        zsh_debug_echo "    Status: ❌ Missing"
    fi

    zsh_debug_echo "  Cache Dir: $ZSH_COMPLETION_CACHE_DIR"
    if [[ -d "$ZSH_COMPLETION_CACHE_DIR" ]]; then
        local cache_files=$(find "$ZSH_COMPLETION_CACHE_DIR" -type f 2>/dev/null | wc -l)
        zsh_debug_echo "    Status: ✅ Exists ($cache_files cache files)"
    else
        zsh_debug_echo "    Status: ❌ Missing"
    fi

    zsh_debug_echo ""
    zsh_debug_echo "Completion Directories in fpath:"
    local comp_dirs=0
    for dir in $fpath; do
        if [[ "$dir" == *"completion"* ]] || [[ "$dir" == *"zsh/site-functions"* ]]; then
            if [[ -d "$dir" ]]; then
                local func_count=$(find "$dir" -name "_*" -type f 2>/dev/null | wc -l)
                zsh_debug_echo "  ✅ $dir ($func_count functions)"
    ...
    zsh_debug_echo ""
    zsh_debug_echo "Old .zcompdump Files:"
    local old_files=0
...
for file in $~pattern(N); do
    ...
    if [[ $old_files -eq 0 ]]; then
        zsh_debug_echo "  ✅ No old .zcompdump files found"
    else
        zsh_debug_echo "  💡 Run 'cleanup-old-completions' to remove old files"
    fi
}

# 3.3. Clean up old completion files
cleanup-old-completions() {
    zsh_debug_echo "🧹 Cleaning up old .zcompdump files..."
    ...
    zsh_debug_echo "✅ Cleanup complete"
}
...
completion_management_main() {
    zsh_debug_echo "========================================================"
    zsh_debug_echo "ZSH Centralized Completion Management"
    zsh_debug_echo "========================================================"
    zsh_debug_echo "Version: $ZSH_COMPLETION_MANAGEMENT_VERSION"
    zsh_debug_echo "Loaded: $ZSH_COMPLETION_MANAGEMENT_LOADED"
    zsh_debug_echo ""
