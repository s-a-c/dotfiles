...
[[ "$ZSH_DEBUG" == "1" ]] && {
        zsh_debug_echo "# ++++++ $0 ++++++++++++++++++++++++++++++++++++"
    zsh_debug_echo "# [git-config] Setting up Git and VCS configurations"
}
...
## [git.hooks] - Git hook helpers
{
    ...
    setup_git_hooks() {
        ...
        if [[ -z "$repo_root" ]]; then
            zsh_debug_echo "Error: Not in a Git repository"
            return 1
        fi
...
if git diff --check --cached; then
    zsh_debug_echo "Error: Trailing whitespace found"
    exit 1
fi
EOF
            chmod +x "$hooks_dir/pre-commit"
            zsh_debug_echo "Pre-commit hook installed"
        fi
...
if ! grep -qE "$commit_regex" "$1"; then
    zsh_debug_echo "Invalid commit message format"
    zsh_debug_echo "First line should be 50 characters or less"
    exit 1
fi
EOF
            chmod +x "$hooks_dir/commit-msg"
            zsh_debug_echo "Commit-msg hook installed"
        fi
    }
}
...
## [git.worktree] - Git worktree helpers
{
    ...
    git-worktree-create() {
        if [[ $# -lt 1 ]]; then
            zsh_debug_echo "Usage: gwt <branch-name> [path]"
            zsh_debug_echo "Creates a new git worktree for the given branch"
            return 1
        fi
        ...
        zsh_debug_echo "Worktree created at: $path"
    }
    ...
    gwtr() {
        if [[ $# -lt 1 ]]; then
            zsh_debug_echo "Usage: gwtr <path>"
            return 1
        fi
...
## [git.integration] - Integration with other tools
{
    ...
    if command -v gh >/dev/null 2>&1; then
        ...
        gissue() {
            if [[ -z "$1" ]]; then
                zsh_debug_echo "Usage: gissue <title> [body]"
                return 1
            fi
