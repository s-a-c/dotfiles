...
create_lazy_loader() {
    ...
    # Create the lazy loading function
    {
        ...
        lazy_load_${plugin_key//-/_}() {
            [[ -n \"\$ZSH_DEBUG\" ]] && zsh_debug_echo \"# [lazy-load] Loading plugin: $plugin_spec\"
        ...
        # Create placeholder function
        {
            ...
            $cmd() {
                ...
                if command -v '$cmd' >/dev/null 2>&1; then
                    ...
                    zsh_debug_echo 'Command $cmd not available after loading plugin'
                    return 1
                fi
            }
            "
...
load_lazy_plugin() {
    ...
    if [[ -n "${_LAZY_PLUGIN_REGISTRY[$plugin_name]}" ]]; then
        ...
        zsh_debug_echo "Unknown lazy plugin: $plugin_name"
        zsh_debug_echo "Available plugins: ${(@k)_LAZY_PLUGIN_REGISTRY}"
        return 1
    fi
}
