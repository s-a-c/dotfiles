...
initialize_starship_prompt() {
    ...
    _post_debug "✅ Starship available"
    
    # Check if ZLE is ready
    if [[ "${ZLE_READY:-0}" != "1" ]]; then
        _post_debug "⚠️  ZLE not ready, deferring Starship initialization"
        _post_debug "ZLE Status: ${ZLE_INIT_STATUS:-not_available}"
        return 1
    fi
    
    _post_debug "✅ ZLE ready, initializing Starship..."
    ...
    if [[ -n "$starship_init" ]]; then
        ...
        if eval "$starship_init" 2>/dev/null; then
            if [[ -n "${STARSHIP_SHELL:-}" ]]; then
                ...
                _post_debug "⚠️  Starship eval succeeded but STARSHIP_SHELL not set"
                export STARSHIP_SHELL="zsh"
                export STARSHIP_INITIALIZED="1"
                _post_debug "✅ STARSHIP_SHELL manually set"
                return 0
            fi
        else
