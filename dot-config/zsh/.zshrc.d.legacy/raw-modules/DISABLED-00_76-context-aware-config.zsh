...
if [[ -z "${ZSH_SOURCE_EXECUTE_LOADED:-}" ]]; then
    ...
    if [[ -f "$detection_script" ]]; then
        ...
        zsh_debug_echo "Context-aware config will work but without context-aware features"
    fi
fi
...
if declare -f context_zsh_debug_echo >/dev/null 2>&1; then
    ...
    _context_error() {
        ...
        if declare -f handle_error >/dev/null 2>&1; then
            ...
            zsh_debug_echo "ERROR [context]: $message"
    ...
    _context_log() {
        ...
        local L=${level:u}
        if [[ "$L" == "ERROR" || "$L" == "WARN" || "$ZSH_STARTUP_VERBOSE" == "true" || "$ZSH_DEBUG" == "1" ]]; then
            zsh_debug_echo "# [context][$L] $msg"
        fi
    }
    _context_error() {
        zsh_debug_echo "ERROR [context]: $1"
        return "${2:-1}"
    }
fi
...
export ZSH_CONTEXT_AWARE_LOADED="$(command -v date >/dev/null && date -u '+    %FT%T %Z' || zsh_debug_echo 'loaded')"
    ...
    if source "$config_file"; then
        ...
        if command -v date >/dev/null 2>&1; then
            ...
            if command -v bc >/dev/null 2>&1; then
                load_duration=$(zsh_debug_echo "scale=3; ($load_end_time - $load_start_time) * 1000" | bc 2>/dev/null || zsh_debug_echo "unknown")
            else
                load_duration="<1"
            fi
        else
        ...
        if command -v env >/dev/null 2>&1 && command -v sort >/dev/null 2>&1; then
            ...
            if command -v comm >/dev/null 2>&1 && command -v wc >/dev/null 2>&1 && [[ -n "$env_before" ]]; then
                env_changes=$(comm -13 <(zsh_debug_echo "$env_before") <(zsh_debug_echo "$env_after") | wc -l)
            else
                env_changes="unknown"
            fi
        else
...
_apply_context_configuration() {
    ...
    zsh_debug_echo "$dir" > "$ZSH_CONTEXT_CURRENT_FILE" 2>/dev/null || true
...
context-status() {
    zsh_debug_echo "========================================================"
    zsh_debug_echo "Context-Aware Configuration Status"
    zsh_debug_echo "========================================================"
    zsh_debug_echo "Version: $ZSH_CONTEXT_AWARE_VERSION"
    zsh_debug_echo "Enabled: $ZSH_CONTEXT_ENABLE"
    zsh_debug_echo "Current Directory: $PWD"
    zsh_debug_echo "Previous Directory: ${ZSH_CONTEXT_PREVIOUS_DIR:-none}"
    zsh_debug_echo ""

    zsh_debug_echo "Detected Contexts:"
...
context-reload() {
    ...
    zsh_debug_echo "Context configuration reloaded"
}
...
context-create() {
    ...
    if [[ -z "$context_name" ]]; then
        zsh_debug_echo "Usage: context-create <context-name>"
        zsh_debug_echo "Example: context-create nodejs"
        return 1
    fi
    ...
    if [[ -f "$config_file" ]]; then
        zsh_debug_echo "Context configuration already exists: $config_file"
        return 1
    fi
...
# Auto-generated on $(date -u '+    %FT%T %Z')
...
zsh_debug_echo "Loaded $context_name context configuration"
EOF

    zsh_debug_echo "Created context configuration: $config_file"
    zsh_debug_echo "Edit the file to customize the configuration for $context_name contexts"
}
...
context_main() {
    zsh_debug_echo "========================================================"
    zsh_debug_echo "ZSH Context-Aware Dynamic Configuration System"
    zsh_debug_echo "========================================================"
    zsh_debug_echo "Version: $ZSH_CONTEXT_AWARE_VERSION"
    zsh_debug_echo "Loaded: $ZSH_CONTEXT_AWARE_LOADED"
    zsh_debug_echo ""
