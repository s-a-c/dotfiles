...
if [[ -z "${ZSH_SOURCE_EXECUTE_LOADED:-}" ]]; then
    ...
    if [[ -f "$detection_script" ]]; then
        ...
        zsh_debug_echo "Security check will work but without context-aware features"
    fi
fi
...
if declare -f context_zsh_debug_echo >/dev/null 2>&1; then
    ...
    _security_error() {
        ...
        if declare -f handle_error >/dev/null 2>&1; then
            ...
            zsh_debug_echo "ERROR [security]: $message"
    ...
    _security_log() {
        zsh_debug_echo "# [security] $1"
    }
    _security_error() {
        zsh_debug_echo "ERROR [security]: $1"
        return "${2:-1}"
    }
fi
...
export ZSH_SECURITY_CHECK_LOADED="$(date -u '+    %FT%T %Z')"
...
_run_security_audit() {
    ...
    if command -v bc >/dev/null 2>&1; then
        audit_duration=$(zsh_debug_echo "$audit_end_time - $audit_start_time" | bc 2>/dev/null || zsh_debug_echo "0.001")
    else
...
# 6.1. Auto-audit (unless in 040-testing mode)
...
_security_check_main() {
    zsh_debug_echo "========================================================"
    zsh_debug_echo "ZSH Configuration Security Audit System"
    zsh_debug_echo "========================================================"
    zsh_debug_echo "Version: $ZSH_SECURITY_CHECK_VERSION"
    zsh_debug_echo "Loaded: $ZSH_SECURITY_CHECK_LOADED"
    zsh_debug_echo ""
    ...
    zsh_debug_echo "Running comprehensive security audit..."
    if _run_security_audit; then
        zsh_debug_echo ""
        zsh_debug_echo "🎉 Security audit completed successfully!"
        ...
        zsh_debug_echo ""
        zsh_debug_echo "❌ Security audit completed with issues."
        if declare -f safe_exit >/dev/null 2>&1; then
            safe_exit 1
        else
            exit 1
...
elif [[ "${(%):-%N}" == "$0" ]] || [[ "${(%):-%N}" == *"security-check"* ]]; then
