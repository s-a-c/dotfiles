...
if command -v psql >/dev/null 2>&1; then
    ...
    [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] PostgreSQL configured" >&2
fi

if command -v mysql >/dev/null 2>&1; then
    export MYSQL_PS1="(\u@\h) [\d]> "
    [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] MySQL configured" >&2
fi
...
if command -v git >/dev/null 2>&1; then
    ...
    [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] Git tools configured" >&2
fi
...
case "$(uname -s)" in
    Darwin)
        ...
        [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] macOS optimizations applied" >&2
        ;;
    Linux)
        ...
        [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] Linux optimizations applied" >&2
        ;;
    MINGW32_NT*|MINGW64_NT*)
        # Windows/Git Bash optimizations
        export MSYS2_PATH_TYPE=inherit
        [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] Windows optimizations applied" >&2
        ;;
esac
...
if command -v composer >/dev/null 2>&1; then
    ...
    unset composer_home
    [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] Composer configured" >&2
fi
...
if command -v bob >/dev/null 2>&1; then
    ...
    [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] Bob (Neovim manager) configured" >&2
fi

# Bun completions and environment
if [[ -s "${XDG_DATA_HOME}/bun/_bun" ]]; then
    source "${XDG_DATA_HOME}/bun/_bun"
    [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] Bun completions loaded" >&2
fi

# Docker Desktop completions
if [[ -d "$HOME/.docker/completions" ]]; then
    fpath=("$HOME/.docker/completions" $fpath)
    [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] Docker completions added to fpath" >&2
fi
...
if [[ -d "$HOME/google-cloud-sdk" ]]; then
    ...
    [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] Google Cloud SDK integrated" >&2
fi

# MCP Environment Setup for AugmentCode
if [[ -f "$HOME/.mcp-environment.sh" ]]; then
    builtin source "$HOME/.mcp-environment.sh"
    [[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] MCP AugmentCode environment loaded" >&2
fi

## [tool-environments.path-helpers] - PATH management utilities
# PATH management functions are defined in 00-core/04-functions-core.zsh
...
[[ "$ZSH_DEBUG" == "1" ]] && echo "# [tool-environments] ✅ All development tools configured and optimized" >&2
