#!/usr/bin/env zsh
# LEGACY STUB: 70-ssh-agent.zsh (migrated to 200-ssh-agent.zsh)
return 0
...
# Use direct zf:: debug calls (with SSH debug flag check)
_ssh_agent_debug() {
    ((ZSH_SSH_AGENT_DEBUG)) && zf::debug "[SSH] $1"
}
...
ensure_ssh_agent() {
    ...
    if _ssh_agent_valid; then
        ...
        zf::debug "# [ssh-agent] reuse existing ($SSH_AUTH_SOCK)"
        return 0
    fi
    ...
    if _spawn_ssh_agent; then
        ...
        if _ssh_agent_valid; then
            ...
            _SSH_AGENT_VALIDATED=1
            zf::debug "# [ssh-agent] spawn new (pid=$SSH_AGENT_PID sock=$SSH_AUTH_SOCK)"
            return 0
        else
            _SSH_AGENT_LAST_ACTION="failed"
            zf::debug "# [ssh-agent] spawn validation failed"
        ...
        _SSH_AGENT_LAST_ACTION="failed"
        zf::debug "# [ssh-agent] spawn attempt failed"
        return 1
    fi
}

# Backwards-compatible invocation on load (now using zf:: namespace)
zf::ssh_ensure_agent

# ==============================================================================
# NAMESPACE MIGRATION COMPLETE
# ==============================================================================
# All functions migrated to zf:: namespace. Use zf::ssh_* functions directly.
# Note: ensure_ssh_agent() kept for critical SSH functionality

ensure_ssh_agent() { zf::ssh_ensure_agent "$@"; }  # Keep for SSH scripts
