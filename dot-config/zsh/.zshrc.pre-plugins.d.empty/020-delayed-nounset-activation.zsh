#!/usr/bin/env zsh
# 095-DELAYED-NOUNSET-ACTIVATION.ZSH
...
if [[ "${_ZQS_NOUNSET_DISABLED_FOR_OMZ:-0}" == "1" ]]; then
  ...
  if [[ ${ZSH_DEBUG:-0} == 1 && -n ${ZSH_DEBUG_LOG:-} ]]; then
    print -r -- "[NOUNSET-SAFETY][095] nounset kept disabled for Oh-My-Zsh compatibility" >>"$ZSH_DEBUG_LOG" 2>/dev/null || true
  fi
  return 0
fi
...
if [[ ${ZSH_DEBUG:-0} == 1 && -n ${ZSH_DEBUG_LOG:-} ]]; then
  print -r -- "[NOUNSET-SAFETY][095] delayed nounset activation complete" >>"$ZSH_DEBUG_LOG" 2>/dev/null || true
fi
