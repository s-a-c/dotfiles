...
if [[ -d "$HOME/.local/share/bun/bin" ]] && [[ ! "$PATH" == *"$HOME/.local/share/bun/bin"* ]]; then
    ...
    [[ "$ZSH_DEBUG" == "1" ]] && zsh_debug_echo "# Added Bun to PATH for zsh-abbr dependencies"
fi
...
if [[ -n "${NVM_DIR:-}" ]] || [[ -f "$HOME/.nvm/nvm.sh" ]] || [[ -f "/usr/local/opt/nvm/nvm.sh" ]] || [[ -f "/opt/homebrew/opt/nvm/nvm.sh" ]]; then
    ...
    if [[ -n "$NPM_CONFIG_PREFIX" ]]; then
        ...
        [[ "$ZSH_DEBUG" == "1" ]] && zsh_debug_echo "# Backing up and unsetting NPM_CONFIG_PREFIX for NVM compatibility"
        unset NPM_CONFIG_PREFIX
    fi
    [[ "$ZSH_DEBUG" == "1" ]] && zsh_debug_echo "# NVM detected - NPM_CONFIG_PREFIX kept unset for compatibility"
else
    # No NVM detected, safe to set NPM_CONFIG_PREFIX for global package management
    if [[ -z "$NPM_CONFIG_PREFIX" ]]; then
        export NPM_CONFIG_PREFIX="$HOME/.local/share/npm"
        [[ "$ZSH_DEBUG" == "1" ]] && zsh_debug_echo "# No NVM detected - NPM_CONFIG_PREFIX set to $NPM_CONFIG_PREFIX"
    fi
fi
