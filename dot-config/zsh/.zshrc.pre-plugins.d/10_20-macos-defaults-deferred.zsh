...
_deferred_macos_defaults() {
    ...
    _log_and_execute() {
        zsh_debug_echo "=============================================================================="
        zsh_debug_echo "Deferred macOS Defaults Wrapper Execution"
        zsh_debug_echo "Started: $(date -u +%Y-%m-%dT%H:%M:%SZ)"
        [[ -n "$log_file" ]] && zsh_debug_echo "Log file: $log_file"
        zsh_debug_echo "=============================================================================="
        zsh_debug_echo ""

        # 2.3.4 Check if setup script exists
        if [[ ! -x "$setup_script" ]]; then
            zsh_debug_echo "❌ Setup script not found or not executable: $setup_script"
            return 1
        fi
        ...
        if [[ ! -f "$marker_file" ]]; then
            ...
            if [[ "$setup_script" -nt "$marker_file" ]]; then
                ...
                local marker_timestamp=$(stat -f %m "$marker_file" 2>/dev/null || zsh_debug_echo "0")
        ...
        if [[ "$needs_run" == "true" ]]; then
            zsh_debug_echo "🔄 Running macOS defaults setup - $run_reason"
            zsh_debug_echo ""
            ...
            if "$setup_script"; then
                ...
                zsh_debug_echo ""
                zsh_debug_echo "✅ macOS defaults setup completed successfully"
                zsh_debug_echo "📄 Marker file updated: $marker_file"
            else
                local exit_code=$?
                zsh_debug_echo ""
                zsh_debug_echo "❌ macOS defaults setup failed with exit code: $exit_code"
                return 1
            fi
        else
            zsh_debug_echo "⏭️  Skipping macOS defaults setup - already up to date"
            zsh_debug_echo "📄 Last run: $(cat "$marker_file" 2>/dev/null || zsh_debug_echo "Unknown")"
        fi

        zsh_debug_echo ""
        zsh_debug_echo "=============================================================================="
        zsh_debug_echo "Deferred macOS defaults wrapper completed at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
        zsh_debug_echo "=============================================================================="
    }
