...
[[ "$ZSH_DEBUG" == "1" ]] && {
        zsh_debug_echo "# ++++++ $0 ++++++++++++++++++++++++++++++++++++"
    zsh_debug_echo "# [security] Loading core SSH agent management"
}
...
_secure_ssh_setup() {
    ...
    [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] Starting secure SSH agent setup ($operation)"

    # 1.2.1. Validate environment safety
    if ! _validate_ssh_environment; then
        zsh_debug_echo "# [secure-ssh] ⚠️  Environment validation failed, skipping SSH agent setup"
        return 1
    fi
    ...
    case "$operation" in
        ...
        "auto"|*)
            if ! _is_ssh_agent_usable; then
                ...
                [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ✅ Using existing SSH agent"
            fi
            ;;
    esac
}
...
_validate_ssh_environment() {
    ...
    if [[ ! -d "${HOME}/.ssh" ]]; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ SSH directory does not exist"
        return 1
    fi
    ...
    if [[ "$ssh_perms" != "700" ]]; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ⚠️  SSH directory permissions are $ssh_perms (should be 700)"
        # Auto-fix permissions
        chmod 700 "${HOME}/.ssh" 2>/dev/null
        if [[ $? -ne 0 ]]; then
            [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ Failed to fix SSH directory permissions"
            return 1
        fi
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ✅ Fixed SSH directory permissions to 700"
    fi

    # 1.3.3. Check for required commands
    if ! command -v ssh-agent >/dev/null 2>&1; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ ssh-agent command not found"
        return 1
    fi

    if ! command -v ssh-add >/dev/null 2>&1; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ ssh-add command not found"
        return 1
    fi

    [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ✅ Environment validation passed"
    return 0
}
...
_is_ssh_agent_usable() {
    ...
    if [[ -z "$SSH_AUTH_SOCK" ]] || [[ -z "$SSH_AGENT_PID" ]]; then
        if [[ -f "$SSH_AGENT_ENV_FILE" ]]; then
            [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] Loading SSH environment from $SSH_AGENT_ENV_FILE"
    ...
    if [[ -z "$SSH_AUTH_SOCK" ]] || [[ -z "$SSH_AGENT_PID" ]]; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ SSH agent environment variables not set"
        return 1
    fi

    # 1.4.3. Check if the socket exists and is accessible
    if [[ ! -S "$SSH_AUTH_SOCK" ]]; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ SSH agent socket does not exist: $SSH_AUTH_SOCK"
        return 1
    fi

    # 1.4.4. Check if the agent process is actually running
    if ! kill -0 "$SSH_AGENT_PID" 2>/dev/null; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ SSH agent process $SSH_AGENT_PID is not running"
        return 1
    fi
    ...
    if ! ssh-add -l >/dev/null 2>&1; then
        ...
        if [[ $exit_code -eq 2 ]]; then
            [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ⚠️  SSH agent is running but cannot connect"
            return 1
        elif [[ $exit_code -eq 1 ]]; then
            [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ✅ SSH agent is running (no keys loaded)"
        else
            [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ✅ SSH agent is running with keys"
        fi
    fi
    ...
    if [[ -f "$SSH_AGENT_ENV_FILE" ]]; then
        ...
        if [[ $env_age -gt $SSH_AGENT_MAX_AGE ]]; then
            [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ⚠️  SSH agent is older than $SSH_AGENT_MAX_AGE seconds, will recreate"
            return 1
        fi
    fi

    [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ✅ SSH agent is usable"
    return 0
}
...
_create_new_ssh_agent() {
    ...
    [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] Creating new SSH agent"
    ...
    # 1.5.3. Save and load environment
        zsh_debug_echo "$agent_output" > "$SSH_AGENT_ENV_FILE"
    source "$SSH_AGENT_ENV_FILE"

    # 1.5.4. Add key if available
    if [[ -f "$SSH_KEY_PATH" ]]; then
        ssh-add "$SSH_KEY_PATH" >/dev/null 2>&1
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ✅ SSH key added"
    fi

    [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ✅ New SSH agent created"
    return 0
}
...
_cleanup_existing_agents() {
    ...
    [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] Basic cleanup of existing agents"
...
secure_ssh_restart() {
    zsh_debug_echo "# [secure-ssh] Restarting SSH agent..."
...
[[ "$ZSH_DEBUG" == "1" ]] && {
    zsh_debug_echo "# [security] Core SSH agent management loaded"
    printf "# ------ %s --------------------------------\n" "$0"
}
