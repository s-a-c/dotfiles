...
[[ "$ZSH_DEBUG" == "1" ]] && {
        zsh_debug_echo "# ++++++ $0 ++++++++++++++++++++++++++++++++++++"
    zsh_debug_echo "# [security] Loading advanced SSH agent security"
}
...
_acquire_ssh_lock() {
    ...
    while (( counter < timeout )); do
        if (set -C;     zsh_debug_echo $$ > "$SSH_AGENT_LOCK_FILE") 2>/dev/null; then
            return 0
        fi
        ...
        if [[ -f "$SSH_AGENT_LOCK_FILE" ]]; then
            local lock_age
            lock_age=$(( $(date +%s) - $(stat -f "%m" "$SSH_AGENT_LOCK_FILE" 2>/dev/null || zsh_debug_echo 0) ))
            if (( lock_age > 30 )); then
...
_validate_agent_output() {
    ...
    if [[ ! -f "$env_file" ]] || [[ ! -r "$env_file" ]]; then
        zsh_debug_echo "# [secure-ssh] ❌ Agent environment file not accessible"
        return 1
    fi
    ...
    if (( line_count > 10 )); then
        zsh_debug_echo "# [secure-ssh] ❌ Agent environment file too large ($line_count lines)"
        return 1
    fi

    # 1.7.3. Validate SSH_AUTH_SOCK and SSH_AGENT_PID format
    if ! grep -q '^SSH_AUTH_SOCK=/.*; export SSH_AUTH_SOCK;$' "$env_file"; then
        zsh_debug_echo "# [secure-ssh] ❌ Invalid SSH_AUTH_SOCK format in agent output"
        return 1
    fi

    if ! grep -q '^SSH_AGENT_PID=[0-9]*; export SSH_AGENT_PID;$' "$env_file"; then
        zsh_debug_echo "# [secure-ssh] ❌ Invalid SSH_AGENT_PID format in agent output"
        return 1
    fi
...
_cleanup_existing_agents_advanced() {
    ...
    [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] Advanced cleanup of existing agents"

    # 1.8.1. Kill existing agent if PID is known
    if [[ -n "$SSH_AGENT_PID" ]] && kill -0 "$SSH_AGENT_PID" 2>/dev/null; then
        kill "$SSH_AGENT_PID" 2>/dev/null
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] Killed existing agent (PID: $SSH_AGENT_PID)"
    fi
...
_add_ssh_key_secure() {
    ...
    if [[ ! -f "$SSH_KEY_PATH" ]]; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ SSH key not found: $SSH_KEY_PATH"
        return 1
    fi
    ...
    if [[ "$key_perms" != "600" ]]; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ⚠️  SSH key permissions are $key_perms (should be 600)"
        chmod 600 "$SSH_KEY_PATH" 2>/dev/null
        if [[ $? -ne 0 ]]; then
            [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ Failed to fix SSH key permissions"
            return 1
        fi
    fi
    ...
    if timeout 10 ssh-add "$SSH_KEY_PATH" >/dev/null 2>&1; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ✅ SSH key added successfully"
        return 0
    fi

    zsh_debug_echo "# [secure-ssh] ❌ Failed to add SSH key"
    return 1
}

# 1.10. Advanced Public Management Functions
secure_ssh_restart_advanced() {
    zsh_debug_echo "# [secure-ssh] Advanced restart with full cleanup..."
    ...
    if _acquire_ssh_lock; then
        ...
        zsh_debug_echo "# [secure-ssh] ❌ Could not acquire lock for restart"
        return 1
    fi
}
...
secure_ssh_status() {
    ...
    if [[ -n "$SSH_AUTH_SOCK" ]]; then
            ...
            zsh_debug_echo "SSH_AUTH_SOCK: (not set)"
    fi

    if [[ -n "$SSH_AGENT_PID" ]]; then
            zsh_debug_echo "SSH_AGENT_PID: $SSH_AGENT_PID"
        if kill -0 "$SSH_AGENT_PID" 2>/dev/null; then
                zsh_debug_echo "Agent Process: ✅ Running"
        else
                zsh_debug_echo "Agent Process: ❌ Not running"
        fi
    else
            zsh_debug_echo "SSH_AGENT_PID: (not set)"
    fi

    if [[ -f "$SSH_AGENT_ENV_FILE" ]]; then
        local env_age=$(( $(date +%s) - $(stat -f %m "$SSH_AGENT_ENV_FILE" 2>/dev/null || zsh_debug_echo 0) ))
            zsh_debug_echo "Environment File: ✅ Exists (${env_age}s old)"
    else
            zsh_debug_echo "Environment File: ❌ Missing"
    fi

        zsh_debug_echo ""
        zsh_debug_echo "Loaded Keys:"
...
secure_ssh_add_key() {
    ...
    if [[ ! -f "$key_path" ]]; then
            zsh_debug_echo "❌ Key not found: $key_path"
        return 1
    fi

    if _add_ssh_key_secure; then
            zsh_debug_echo "✅ Key added successfully: $key_path"
    else
            zsh_debug_echo "❌ Failed to add key: $key_path"
        return 1
    fi
}
...
secure_ssh_remove_all_keys() {
    if ssh-add -D >/dev/null 2>&1; then
            zsh_debug_echo "✅ All keys removed from agent"
    else
            zsh_debug_echo "❌ Failed to remove keys from agent"
        return 1
    fi
}
...
_create_new_ssh_agent_secure() {
    ...
    [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] Creating new SSH agent with full security"

    # 1.11.1. Acquire lock to prevent race conditions
    if ! _acquire_ssh_lock; then
        zsh_debug_echo "# [secure-ssh] ❌ Could not acquire lock for agent creation"
        return 1
    fi
    ...
    if [[ $? -ne 0 ]] || [[ -z "$agent_output" ]]; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ Failed to start SSH agent"
        _release_ssh_lock
        return 1
    fi
    ...
    local temp_env_file="${SSH_AGENT_ENV_FILE}.tmp"
        zsh_debug_echo "$agent_output" > "$temp_env_file"

    # 1.11.5. Validate agent output
    if ! _validate_agent_output "$temp_env_file"; then
        [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ❌ Agent output validation failed"
        rm -f "$temp_env_file"
        _release_ssh_lock
        return 1
    fi
    ...
    [[ "$debug_mode" == "1" ]] && zsh_debug_echo "# [secure-ssh] ✅ Secure SSH agent created successfully"
    return 0
}
...
[[ "$ZSH_DEBUG" == "1" ]] && {
    zsh_debug_echo "# [security] Advanced SSH agent security loaded"
    printf "# ------ %s --------------------------------\n" "$0"
}
