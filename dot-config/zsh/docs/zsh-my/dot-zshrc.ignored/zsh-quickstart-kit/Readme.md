...
1. [ZSH Quickstart Kit](#zsh-quickstart-kit)
   1. [Table of Contents](#table-of-contents)
   1. [Announcement](#announcement)
   1. [Installation](#installation)
      1. [Prerequisites](#prerequisites)
         1. [Fonts](#fonts)
      1. [OS-specific setup](#os-specific-setup)
         1. [fzf](#fzf)
         1. [macOS](#macos)
         1. [Linux](#linux)
      1. [Set up Zgenom and the starter kit](#set-up-zgenom-and-the-starter-kit)
   1. [Contents of the kit](#contents-of-the-kit)
      1. [Included plugins](#included-plugins)
   1. [Customizing the kit](#customizing-the-kit)
      1. [Behavior toggles](#behavior-toggles)
         1. [zqs](#zqs)
            1. [zqs check-for-updates](#zqs-check-for-updates)
            1. [zqs disable-bindkey-handling](#zqs-disable-bindkey-handling)
            1. [zqs disable-1password-agent](#zqs-disable-1password-agent)
            1. [zqs enable-1password-agent](#zqs-enable-1password-agent)
            1. [zqs enable-bindkey-handling](#zqs-enable-bindkey-handling)
            1. [zqs disable-diff-so-fancy](#zqs-disable-diff-so-fancy)
            1. [zqs enable-diff-so-fancy](#zqs-enable-diff-so-fancy)
            1. [zqs disable-omz-plugins](#zqs-disable-omz-plugins)
            1. [zqs enable-control-c-decorator](#zqs-enable-control-c-decorator)
            1. [zqs disable-control-c-decorator](#zqs-disable-control-c-decorator)
            1. [zqs enable-omz-plugins](#zqs-enable-omz-plugins)
            1. [zqs enable-ssh-askpass-require](#zqs-enable-ssh-askpass-require)
            1. [zqs disable-ssh-askpass-require](#zqs-disable-ssh-askpass-require)
            1. [zqs disable-ssh-key-listing](#zqs-disable-ssh-key-listing)
            1. [zqs enable-ssh-key-listing](#zqs-enable-ssh-key-listing)
            1. [zqs disable-ssh-key-loading](#zqs-disable-ssh-key-loading)
            1. [zqs enable-ssh-key-loading](#zqs-enable-ssh-key-loading)
            1. [zqs disable-zmv-autoloading](#zqs-disable-zmv-autoloading)
            1. [zqs enable-zmv-autoloading](#zqs-enable-zmv-autoloading)
            1. [zqs disable-zsh-profiling](#zqs-disable-zsh-profiling)
            1. [zqs enable-zsh-profiling](#zqs-enable-zsh-profiling)
            1. [zqs selfupdate](#zqs-selfupdate)
            1. [zqs update](#zqs-update)
            1. [zqs update-plugins](#zqs-update-plugins)
            1. [zqs cleanup](#zqs-cleanup)
            1. [zqs get-setting](#zqs-get-setting)
            1. [zqs set-setting](#zqs-set-setting)
            1. [zqs delete-setting](#zqs-delete-setting)
      1. [Functions and Aliases](#functions-and-aliases)
         1. [Customizing with ~/.zshrc.d](#customizing-with-zshrcd)
      1. [I like a plugin, but some of the aliases and functions it installs overwrite other commands or aliases I use](#i-like-a-plugin-but-some-of-the-aliases-and-functions-it-installs-overwrite-other-commands-or-aliases-i-use)
      1. [ZSH options](#zsh-options)
         1. [Fragment file directories](#fragment-file-directories)
      1. [Self-update Settings](#self-update-settings)
      1. [Customizing the plugin list](#customizing-the-plugin-list)
         1. [Using fragment files](#using-fragment-files)
         1. [Complete plugin list replacement](#complete-plugin-list-replacement)
      1. [Disabling zmv](#disabling-zmv)
      1. [Disabling oh-my-zsh](#disabling-oh-my-zsh)
   1. [FAQ](#faq)
      1. [How do I reconfigure the prompt?](#how-do-i-reconfigure-the-prompt)
      1. [Powerlevel 10k warns that there is console output during startup](#powerlevel-10k-warns-that-there-is-console-output-during-startup)
      1. [I added a new completion plugin, and it isn't working](#i-added-a-new-completion-plugin-and-it-isnt-working)
      1. [I get a git error when I try to update the kit](#i-get-a-git-error-when-i-try-to-update-the-kit)
      1. [GNU stow is warning that stowing zsh would cause conflicts](#gnu-stow-is-warning-that-stowing-zsh-would-cause-conflicts)
      1. [\_arguments:comparguments:325: can only be called from completion function](#_argumentscomparguments325-can-only-be-called-from-completion-function)
      1. [Could not open a connection to your authentication agent](#could-not-open-a-connection-to-your-authentication-agent)
      1. [I want to pin a plugin version](#i-want-to-pin-a-plugin-version)
   1. [Other Resources](#other-resources)
      1. [ZSH](#zsh)
      1. [Dotfiles in general](#dotfiles-in-general)
      1. [Vim](#vim)
   1. [Thanks](#thanks)
