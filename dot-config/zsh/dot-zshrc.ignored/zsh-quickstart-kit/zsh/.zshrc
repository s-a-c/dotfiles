...
load-our-ssh-keys() {
  ...
  if [ $($key_manager -l | grep -c "The agent has no identities." ) -eq 1 ]; then
    ...
    for key in $(find $ZDOTDIR/.ssh -type f -a \( -name '*id_rsa' -o -name '*id_dsa' -o -name '*id_ecdsa' -o -name '*id_ed25519' \))
    do
...
if [ -f $ZDOTDIR/.aws/aws_variables ]; then
  source $ZDOTDIR/.aws/aws_variables
fi
...
if [[ "$(uname -s)" == "Darwin" ]]; then
  ...
  [ -r $ZDOTDIR/.osx_aliases ] && source $ZDOTDIR/.osx_aliases
