...
_test_date=$(date -u '+%Y-%m-%d')
_test_timestamp=$(date -u '+%Y%m%d_%H%M%S')
...
_log_test() {
    echo "[$(date -u '+%Y-%m-%d %H:%M:%S UTC')] $*" >> "$_test_log_file" 2>&1
    echo "$*"
}
...
echo "WRAPPER_TEST_git:$(type git 2>/dev/null | grep -q 'git is a shell function' && echo 'YES' || echo 'NO')"
echo "WRAPPER_TEST_docker:$(type docker 2>/dev/null | grep -q 'docker is a shell function' && echo 'YES' || echo 'NO')"
echo "WRAPPER_TEST_op:$(type op 2>/dev/null | grep -q 'op is a shell function' && echo 'YES' || echo 'NO')"
echo "WRAPPER_TEST_rake:$(type rake 2>/dev/null | grep -q 'rake is a shell function' && echo 'YES' || echo 'NO')"
...
echo "DEFER_TEST:$(command -v zsh-defer >/dev/null 2>&1 && echo 'AVAILABLE' || echo 'NOT_AVAILABLE')"
EOF

chmod +x "$_temp_test_script"

# Run the test script with timeout to prevent hanging
_test_output=$(timeout 10s zsh "$_temp_test_script" 2>/dev/null || echo "TIMEOUT")
...
if [[ "$_test_output" == "TIMEOUT" ]]; then
    ...
    # Parse test results
    echo "$_test_output" | while IFS=':' read -r test_type result; do
        case "$test_type" in
            WRAPPER_TEST_git)
...
_original_plugin_count=$(grep -c "zgenom load" "$_backup_file" 2>/dev/null || echo "0")
_current_plugin_count=$(grep -c "zgenom load" /Users/s-a-c/.config/zsh/.zgen-setup 2>/dev/null || echo "0")
