...
EOF
    ...
    source "$TEMP_DIR/mock-async-module.zsh"
}
...
print_test "Premature async start detection"
{
    ...
    rm -f "$ASYNC_STATE_LOG" "$PERF_LOG" 2>/dev/null || true
    unset _PERF_PROMPT_COMPLETE _ASYNC_START_TIME

    init_async_security

    # Try to start async without prompt completion
    warning_output=$(start_async_security 2>&1)

    if [[ "$warning_output" == *"Warning: Starting async before prompt completion"* ]]; then
        print_pass
    else
        print_fail "Should warn about premature async start"
    fi
}
...
{
    # Check that all log entries have proper format
    invalid_lines=$(grep -v -E "^(ASYNC_STATE:|SECURITY_ASYNC_QUEUE|PERF_PROMPT:)" "$ASYNC_STATE_LOG" 2>/dev/null | wc -l | tr -d ' ')
