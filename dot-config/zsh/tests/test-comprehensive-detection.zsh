...
#          execute modes with full system integration testing.
#
...
test_detection_system_core_functionality() {
    ...
    for func_name in "${core_functions[@]}"; do
        if declare -f "$func_name" > /dev/null 2>&1; then
            ...
            echo "    ✓ $func_name is available and callable"
        else
            echo "    ✗ $func_name is not available"
        fi
    done

    echo "    📊 Core functionality: $working_functions/$total_functions functions working"
...
test_detection_accuracy_in_current_context() {
    ...
    echo "    Current execution context: $current_context"

    # Test consistency between detection functions
    if is_being_sourced; then
        if is_being_executed; then
            echo "    ✗ Inconsistent detection: both sourced and executed returned true"
            return 1
        else
            echo "    ✓ Consistent detection: sourced=true, executed=false"
        fi
    else
        if is_being_executed; then
            echo "    ✓ Consistent detection: sourced=false, executed=true"
        else
            echo "    ✗ Inconsistent detection: both sourced and executed returned false"
            return 1
        fi
    fi
    ...
    if is_being_sourced && [[ "$current_context" =~ "sourced" ]]; then
        echo "    ✓ Context string matches sourced state"
    elif is_being_executed && [[ "$current_context" =~ "executed" ]]; then
        echo "    ✓ Context string matches executed state"
    else
        echo "    ⚠ Context string may not perfectly match detection state"
...
test_error_handling_cross_context() {
    ...
    echo "    Testing error handling in $context context"
...
test_environment_management_cross_context() {
    ...
    if [[ "$retrieved_value" == "$test_var_value" ]]; then
        echo "    ✓ Environment management working in $(get_execution_context) context"
        unset "$test_var_name"
        return 0
    else
        echo "    ✗ Environment management failed in $(get_execution_context) context"
        unset "$test_var_name"
        return 1
    fi
}
...
test_output_control_cross_context() {
    ...
    if [[ -n "$both_output" ]]; then
        echo "    ✓ 'Both' filter produces output in $context context"
    else
        echo "    ✗ 'Both' filter failed to produce output in $context context"
        return 1
    fi
    ...
    if is_being_sourced; then
        if [[ -n "$sourced_output" && -z "$executed_output" ]]; then
            echo "    ✓ Context-specific filters work correctly in sourced context"
        else
            echo "    ⚠ Context-specific filter behavior unexpected in sourced context"
        fi
    else
        if [[ -z "$sourced_output" && -n "$executed_output" ]]; then
            echo "    ✓ Context-specific filters work correctly in executed context"
        else
            echo "    ⚠ Context-specific filter behavior unexpected in executed context"
        fi
    fi
    ...
    for test_suite in "${test_suites[@]}"; do
        if [[ -f "$test_suite" ]]; then
            ...
            if source "$test_suite" >/dev/null 2>&1; then
                ...
                echo "    ✓ $(basename "$test_suite") can be sourced successfully"
            else
                echo "    ⚠ $(basename "$test_suite") had issues when sourced"
            fi
        fi
    done

    if [[ $total_suites -gt 0 ]]; then
        echo "    📊 Test suite integration: $working_suites/$total_suites suites working"
        [[ $working_suites -ge $((total_suites * 3 / 4)) ]]
    else
        echo "    ⚠ No test suites found for integration testing"
        return 0
    fi
}
...
test_detection_performance() {
    ...
    echo "    📊 Detection performance: 300 function calls in ${duration}s"

    # Performance should be reasonable (less than 1 second for 300 calls)
    if (( $(echo "$duration < 1.0" | bc -l 2>/dev/null || echo "1") )); then
        echo "    ✓ Detection performance is acceptable"
        return 0
    else
        echo "    ⚠ Detection performance may be slow"
...
test_detection_reliability() {
    ...
    echo "    📊 Detection reliability: $consistent_results/$total_tests consistent results"
...
test_logging_system_integration() {
    ...
    if [[ -d "$LOG_DIR" ]]; then
        ...
        if [[ -f "$detection_log" ]]; then
            echo "    ✓ Detection log file exists"
            return 0
        else
            echo "    ⚠ Detection log file not found (may be expected)"
            return 0
        fi
    else
        echo "    ⚠ Log directory not found (may be expected)"
        return 0
    fi
}
...
test_function_export_system() {
    ...
    for func_name in "${available_functions[@]}"; do
        ...
        if declare -f "$func_name" >/dev/null 2>&1; then
            ...
            echo "    ✓ $func_name is available in current shell"
        else
            echo "    ✗ $func_name is not available"
        fi
    done

    echo "    📊 Function availability: $available_count/$total_functions functions available"
...
run_all_tests() {
    echo "========================================================"
    echo "Comprehensive Detection Test Suite"
    echo "========================================================"
    ...
    echo "=== System-wide Detection Validation ==="
    ...
    echo ""
    echo "=== Cross-context Behavior Validation ==="
    ...
    if [[ $TEST_FAILED -eq 0 ]]; then
        echo ""
        echo "🎉 All comprehensive detection tests passed!"
        echo "🎯 Source/Execute Detection System is fully validated!"
        return 0
    else
        echo ""
        echo "❌ $TEST_FAILED comprehensive detection test(s) failed."
        return 1
    fi
}
...
if is_being_executed; then
    ...
    echo "Comprehensive detection test functions loaded (sourced context)"
