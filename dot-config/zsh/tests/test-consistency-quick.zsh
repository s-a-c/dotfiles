...
if [[ $DOUBLE_HASH -eq 0 ]]; then
    echo "✅ No double-hash comments found"
else
    echo "❌ Found $DOUBLE_HASH files with double-hash comments"
    ISSUES=$((ISSUES + 1))
fi
...
if [[ $WRONG_SHEBANG -eq 0 ]]; then
    echo "✅ All shebangs are consistent"
else
    echo "❌ Found $WRONG_SHEBANG inconsistent shebangs"
    ISSUES=$((ISSUES + 1))
fi
...
if [[ $FUNCTION_KEYWORD -eq 0 ]]; then
    echo "✅ No 'function' keyword usage found"
else
    echo "❌ Found $FUNCTION_KEYWORD files using 'function' keyword"
    ISSUES=$((ISSUES + 1))
fi
...
if [[ $TAB_FILES -eq 0 ]]; then
    ...
    echo "❌ Found $TAB_FILES files with tab characters"
    ISSUES=$((ISSUES + 1))
fi
...
if [[ $CONSISTENCY_SCORE -eq 100 ]]; then
    echo "🏆 PERFECT: 100% consistency achieved!"
elif [[ $CONSISTENCY_SCORE -ge 95 ]]; then
    echo "🥇 EXCELLENT: ${CONSISTENCY_SCORE}% consistency achieved!"
elif [[ $CONSISTENCY_SCORE -ge 90 ]]; then
    echo "🥈 VERY GOOD: ${CONSISTENCY_SCORE}% consistency achieved!"
else
    echo "⚠️ NEEDS IMPROVEMENT: ${CONSISTENCY_SCORE}% consistency"
fi
