...
assert_file_contains_pattern() {
    ...
    if [[ ! -f "$file_path" ]]; then
        echo "    ASSERTION FAILED: File does not exist: $file_path"
        return 1
    fi
...
assert_file_has_shebang() {
    ...
    if [[ "$first_line" == "$expected_shebang" ]]; then
        ...
        echo "    ASSERTION FAILED: Incorrect shebang in $file_path"
        echo "    Expected: $expected_shebang"
    ...
    for test_file in "${test_files[@]}"; do
        if [[ -f "$test_file" ]]; then
            ...
            if grep -q "is_being_sourced\|is_being_executed" "$test_file" 2>/dev/null; then
                ...
                echo "    ✓ $test_file uses detection functions"
            else
                echo "    ⚠ $test_file does not use detection functions"
            fi
        fi
    done
    
    echo "    Integration status: $integration_count/$total_files test files use detection"
    ...
    for test_file in "${test_files[@]}"; do
        if [[ -f "$test_file" ]]; then
            ...
            if assert_file_has_shebang "$test_file" "#!/opt/homebrew/bin/zsh" 2>/dev/null; then
                ...
                echo "    ✓ $test_file has correct shebang"
            else
                echo "    ⚠ $test_file has incorrect shebang"
            fi
        fi
    done
    
    echo "    Shebang status: $correct_shebang_count/$total_files test files have correct shebang"
    ...
    for bin_file in "${bin_files[@]}"; do
        if [[ -f "$bin_file" ]]; then
            ...
            if grep -q "is_being_sourced\|is_being_executed\|source.*detection" "$bin_file" 2>/dev/null; then
                integration_count=$((integration_count + 1))
                echo "    ✓ $bin_file uses detection system"
            else
                echo "    ⚠ $bin_file does not use detection system"
            fi
        fi
    done
    
    echo "    Integration status: $integration_count/$total_files bin scripts use detection"
    ...
    for bin_file in "${bin_files[@]}"; do
        if [[ -f "$bin_file" ]]; then
            ...
            if assert_file_has_shebang "$bin_file" "#!/opt/homebrew/bin/zsh" 2>/dev/null; then
                correct_shebang_count=$((correct_shebang_count + 1))
                echo "    ✓ $bin_file has correct shebang"
            else
                echo "    ⚠ $bin_file has incorrect shebang"
            fi
        fi
    done
    
    if [[ $total_files -gt 0 ]]; then
        echo "    Shebang status: $correct_shebang_count/$total_files bin scripts have correct shebang"
        [[ $correct_shebang_count -ge $((total_files / 2)) ]]
    else
        echo "    No applicable bin scripts found"
        return 0
    fi
}
...
test_config_scripts_source_detection() {
    ...
    local config_files=(
        "$ZSHRC_DIR/00-core/00-standard-helpers.zsh"
        "$ZSHRC_DIR/10-tools/15-ssh-agent-macos.zsh"
    )
    ...
    for config_file in "${config_files[@]}"; do
        if [[ -f "$config_file" ]]; then
            ...
            if grep -q "source.*detection\|is_being_sourced\|is_being_executed" "$config_file" 2>/dev/null; then
                integration_count=$((integration_count + 1))
                echo "    ✓ $config_file integrates with detection system"
            else
                echo "    ⚠ $config_file does not integrate with detection system"
            fi
        fi
    done
    
    if [[ $total_files -gt 0 ]]; then
        echo "    Integration status: $integration_count/$total_files config files use detection"
        [[ $integration_count -ge 0 ]]  # Always pass for now, integration is gradual
    else
        echo "    No applicable config files found"
        return 0
    fi
}
...
test_detection_functions_globally_available() {
    ...
    for func_name in "${required_functions[@]}"; do
        if declare -f "$func_name" > /dev/null 2>&1; then
            ...
            echo "    ✓ $func_name is available"
        else
            echo "    ✗ $func_name is not available"
        fi
    done
    
    echo "    Availability status: $available_count/$total_functions functions are globally available"
...
test_detection_system_initialization() {
    ...
    local detection_file="$ZSHRC_DIR/00-core/01-source-execute-detection.zsh"
    
    if [[ ! -f "$detection_file" ]]; then
        echo "    ASSERTION FAILED: Detection system file not found"
        return 1
    fi
    ...
    for script_file in "${script_files[@]}"; do
        if [[ -f "$script_file" ]]; then
            ...
            if grep -q "handle_error\|safe_exit\|context_echo.*ERROR" "$script_file" 2>/dev/null; then
                ...
                echo "    ✓ $script_file uses consistent error handling"
            else
                echo "    ⚠ $script_file may not use consistent error handling"
            fi
        fi
    done
    
    if [[ $total_files -gt 0 ]]; then
        echo "    Consistency status: $consistent_count/$total_files scripts use consistent error handling"
...
run_all_tests() {
    echo "========================================================"
    echo "Detection Integration Test Suite"
    echo "========================================================"
    ...
    echo "=== Test Script Integration Tests ==="
    ...
    echo ""
    echo "=== Helper Script Integration Tests ==="
    ...
    echo ""
    echo "=== Integration Consistency Tests ==="
    ...
    if [[ $TEST_FAILED -eq 0 ]]; then
        echo ""
        echo "🎉 All detection integration tests passed!"
        return 0
    else
        echo ""
        echo "❌ $TEST_FAILED detection integration test(s) failed."
        return 1
    fi
}
...
if is_being_executed; then
    ...
    echo "Detection integration test functions loaded (sourced context)"
