...
test_header_presence() {
    ...
    echo "    📊 Analyzing header presence..."
    ...
    while IFS= read -r config_file; do
        ...
        if echo "$first_10_lines" | grep -q "^#" 2>/dev/null; then
            files_with_headers=$((files_with_headers + 1))
            echo "    ✓ $filename: Has header comments"
        else
            files_without_headers=$((files_without_headers + 1))
            echo "    ⚠ $filename: No header comments"
        fi
    done < <(get_config_files)
    
    echo "    📊 Header presence: $files_with_headers/$total_files files have headers"
...
test_header_format_quality() {
    ...
    echo "    📊 Analyzing header format quality..."
    ...
    while IFS= read -r config_file; do
        ...
        if ! echo "$first_20_lines" | grep -q "^#" 2>/dev/null; then
            continue
        fi
        ...
        if echo "$first_20_lines" | grep -qi "purpose\|description\|configuration" 2>/dev/null; then
        ...
        if echo "$first_20_lines" | grep -qi "file:\|title:\|name:" 2>/dev/null; then
            header_quality_score=$((header_quality_score + 1))
        fi
        
        # Check 5: Has load order or dependency info
        if echo "$first_20_lines" | grep -qi "load\|order\|depend\|phase" 2>/dev/null; then
            header_quality_score=$((header_quality_score + 1))
        fi
        ...
        if [[ $quality_percentage -ge 60 ]]; then
            ...
            echo "    ✓ $filename: Good header quality ($quality_percentage%)"
        else
            files_with_poor_headers=$((files_with_poor_headers + 1))
            echo "    ⚠ $filename: Poor header quality ($quality_percentage%)"
        fi
    done < <(get_config_files)
    
    echo "    📊 Header quality: $files_with_good_headers good, $files_with_poor_headers poor"
...
test_standardized_format_compliance() {
    ...
    echo "    📊 Analyzing standardized format compliance..."
    ...
    while IFS= read -r config_file; do
        ...
        if echo "$first_30_lines" | grep -q "^#.*====\|^#.*----" 2>/dev/null; then
            standard_compliance_score=$((standard_compliance_score + 1))
        fi
        
        # Check 3: Has numbered sections (## 1. or similar)
        if echo "$first_30_lines" | grep -q "^##.*[0-9]\." 2>/dev/null; then
            standard_compliance_score=$((standard_compliance_score + 1))
        fi
        ...
        if [[ $compliance_percentage -ge 50 ]]; then
            ...
            echo "    ✓ $filename: Standard format compliance ($compliance_percentage%)"
        else
            files_without_standard_format=$((files_without_standard_format + 1))
            echo "    ⚠ $filename: Non-standard format ($compliance_percentage%)"
        fi
    done < <(get_config_files)
    
    echo "    📊 Standard format: $files_with_standard_format compliant, $files_without_standard_format non-compliant"
...
test_header_consistency() {
    ...
    echo "    📊 Analyzing header consistency patterns..."
    ...
    if [[ $debug_pattern_files -ge $((total_files * 2 / 3)) ]]; then
        ...
        echo "    ✓ Debug pattern: Consistently used across files"
    else
        echo "    ⚠ Debug pattern: Inconsistent usage"
    fi
    ...
    if [[ $numbered_section_files -ge $((total_files / 3)) ]]; then
        consistent_patterns=$((consistent_patterns + 1))
        echo "    ✓ Section numbering: Used in multiple files"
    else
        echo "    ⚠ Section numbering: Limited usage"
    fi
    ...
    if [[ $separator_files -ge $((total_files / 4)) ]]; then
        consistent_patterns=$((consistent_patterns + 1))
        echo "    ✓ Header separators: Used in multiple files"
    else
        echo "    ⚠ Header separators: Limited usage"
    fi
    
    echo "    📊 Consistency patterns: $consistent_patterns/$total_patterns patterns are consistent"
...
run_all_tests() {
    echo "========================================================"
    echo "Documentation Headers Test Suite"
    echo "========================================================"
    ...
    if [[ $TEST_FAILED -eq 0 ]]; then
        echo ""
        echo "🎉 All documentation header tests passed!"
        return 0
    else
        echo ""
        echo "❌ $TEST_FAILED documentation header test(s) failed."
        return 1
    fi
}
...
if is_being_executed; then
    ...
    echo "Documentation headers test functions loaded (sourced context)"
