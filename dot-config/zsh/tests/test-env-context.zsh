...
test_environment_operation_logging() {
    ...
    if [[ -f "$detection_log" ]]; then
        if grep -q "$test_var_name" "$detection_log" 2>/dev/null; then
            ...
            echo "    WARNING: Environment operation not found in log"
            unset "$test_var_name"
...
run_all_tests() {
    echo "========================================================"
    echo "Environment Context Test Suite"
    echo "========================================================"
    ...
    echo "=== Environment Function Tests ==="
    ...
    if [[ $TEST_FAILED -eq 0 ]]; then
        echo ""
        echo "🎉 All environment context tests passed!"
        return 0
    else
        echo ""
        echo "❌ $TEST_FAILED environment context test(s) failed."
        return 1
    fi
}
...
if is_being_executed; then
    ...
    echo "Environment context test functions loaded (sourced context)"
