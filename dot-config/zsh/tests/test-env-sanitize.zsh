...
SANITIZATION_SCRIPT="${ZDOTDIR:-$HOME/.config/zsh}/.zshrc.d/00-core/08-environment-sanitization.zsh"

if [[ ! -f "$SANITIZATION_SCRIPT" ]]; then
    echo "ERROR: Environment sanitization script not found: $SANITIZATION_SCRIPT" >&2
    exit 1
fi
...
test_sensitive_variable_detection() {
    ...
    for sensitive_var in "${test_vars[@]}"; do
        if ! _is_sensitive_variable "$sensitive_var"; then
            echo "    ASSERTION FAILED: '$sensitive_var' should be detected as sensitive"
            return 1
        fi
    done
    ...
    for safe_var in "${safe_vars[@]}"; do
        if _is_sensitive_variable "$safe_var"; then
            echo "    ASSERTION FAILED: '$safe_var' should not be detected as sensitive"
            return 1
        fi
    done
    
    echo "    ✓ Sensitive variable detection working correctly"
    return 0
}
...
test_sensitive_variable_sanitization() {
    ...
    if [[ -n "${TEST_SECRET_KEY:-}" ]]; then
        echo "    ASSERTION FAILED: TEST_SECRET_KEY should have been sanitized"
        return 1
    fi
    
    if [[ -n "${TEST_API_PASSWORD:-}" ]]; then
        echo "    ASSERTION FAILED: TEST_API_PASSWORD should have been sanitized"
        return 1
    fi
    
    # Check that safe variables were preserved
    if [[ -z "${TEST_SAFE_CONFIG:-}" ]]; then
        echo "    ASSERTION FAILED: TEST_SAFE_CONFIG should have been preserved"
        return 1
    fi
    ...
    echo "    ✓ Sensitive variable sanitization working correctly"
    return 0
}
...
test_path_security_validation() {
    ...
    if ! _validate_path_security >/dev/null 2>&1; then
        echo "    ASSERTION FAILED: Secure PATH should pass validation"
    ...
    if _validate_path_security >/dev/null 2>&1; then
        echo "    ASSERTION FAILED: Insecure PATH with relative entry should fail validation"
        export PATH="$original_path"
        return 1
    fi
    ...
    echo "    ✓ PATH security validation working correctly"
    return 0
}
...
test_umask_enforcement() {
    ...
    if [[ "$new_umask" != "022" ]]; then
        echo "    ASSERTION FAILED: Umask should be set to 022, got $new_umask"
    ...
    echo "    ✓ Umask enforcement working correctly"
    return 0
}
...
test_shell_security_validation() {
    ...
    if ! _validate_shell_security >/dev/null 2>&1; then
        echo "    WARNING: Shell security validation found issues (may be expected)"
        # Don't fail the test as some issues might be environmental
    fi
    
    echo "    ✓ Shell security validation function working"
    return 0
}
...
test_comprehensive_sanitization() {
    ...
    if [[ -n "${TEST_COMPREHENSIVE_SECRET:-}" ]]; then
        echo "    ASSERTION FAILED: Sensitive variable not sanitized"
        test_passed=false
    fi
    
    # Check safe variable was preserved
    if [[ -z "${TEST_COMPREHENSIVE_SAFE:-}" ]]; then
        echo "    ASSERTION FAILED: Safe variable was incorrectly removed"
        test_passed=false
    fi
    
    # Check umask was secured
    if [[ "$(umask)" != "022" ]]; then
        echo "    ASSERTION FAILED: Umask not properly secured"
        test_passed=false
    fi
    ...
    if $test_passed; then
        echo "    ✓ Comprehensive sanitization working correctly"
        return 0
    else
        return 1
    fi
...
test_sanitization_performance() {
    ...
    echo "    📊 Sanitization performance: 5 runs in ${duration}s"
    
    # Performance should be reasonable (less than 1 second for 5 runs)
    if command -v bc >/dev/null 2>&1; then
        if (( $(echo "$duration < 1.0" | bc -l 2>/dev/null || echo "1") )); then
            echo "    ✓ Sanitization performance is acceptable"
            return 0
        else
            echo "    ⚠ Sanitization performance may be slow"
            return 0  # Don't fail test, performance can vary
        fi
    else
        echo "    ✓ Sanitization performance test completed"
        return 0
    fi
}
...
run_all_tests() {
    echo "========================================================"
    echo "Environment Sanitization Test Suite"
    echo "========================================================"
    echo "Timestamp: $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
    echo "Execution Context: $(get_execution_context)"
    echo "Sanitization Version: ${ZSH_ENV_SANITIZATION_VERSION:-unknown}"
    echo ""
    
    log_test "Starting environment sanitization test suite"
    
    # Function Existence Tests
    echo "=== Sanitization Function Tests ==="
    ...
    # Comprehensive Tests
    echo ""
    echo "=== Comprehensive Sanitization Tests ==="
    ...
    if [[ $TEST_FAILED -eq 0 ]]; then
        echo ""
        echo "🎉 All environment sanitization tests passed!"
        return 0
    else
        echo ""
        echo "❌ $TEST_FAILED environment sanitization test(s) failed."
        return 1
    fi
}
...
if is_being_executed; then
    ...
    echo "Environment sanitization test functions loaded (sourced context)"
