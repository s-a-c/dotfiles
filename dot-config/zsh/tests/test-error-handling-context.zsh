...
test_handle_error_message_formatting() {
    ...
    # Capture error output (redirect stderr to stdout for testing)
    local error_output
...
test_error_logging_functionality() {
    ...
    if [[ -f "$detection_log" ]]; then
        if grep -q "$test_error_message" "$detection_log" 2>/dev/null; then
            ...
            echo "    WARNING: Error message not found in detection log"
...
test_cross_context_compatibility() {
    ...
    echo "  Testing in context: $current_context"
...
run_all_tests() {
    echo "========================================================"
    echo "Error Handling Context Test Suite"
    echo "========================================================"
    ...
    # Core Function Tests
    echo "=== Error Handling Function Tests ==="
    ...
    echo ""
    echo "=== Error Context Preservation Tests ==="
    ...
    if [[ $TEST_FAILED -eq 0 ]]; then
        ...
        echo ""
        echo "❌ $TEST_FAILED error handling test(s) failed."
        return 1
    fi
}
...
if is_being_executed; then
    ...
    echo "Error handling context test functions loaded (sourced context)"
