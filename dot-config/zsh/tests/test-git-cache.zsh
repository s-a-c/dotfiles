...
test_cache_refresh_functionality() {
    ...
    local original_timestamp=$(stat -f %m "$cache_file" 2>/dev/null || echo "0")
    ...
    # Get new timestamp
    local new_timestamp=$(stat -f %m "$cache_file" 2>/dev/null || echo "0")
...
main() {
    echo "=============================================================================="
    echo "Git Configuration Caching Test Suite"
    ...
    if [[ -f "$config_base/.zshrc.pre-plugins.d/05-lazy-git-config.zsh" ]]; then
        echo "Loading git caching system..."
        source "$config_base/.zshrc.pre-plugins.d/05-lazy-git-config.zsh"
        echo ""
    else
        echo "❌ Git caching system not found at expected location"
        echo ""
    fi
    
    # Run all tests
    echo "🧪 Running git cache functionality tests..."
    echo ""
    ...
    if [[ $failed_count -gt 0 ]]; then
        echo "❌ Some tests failed. Review the details above."
        echo ""
        echo "Failed Tests:"
        ...
        echo "✅ All tests passed! Git caching system is working correctly."
    fi
    
    echo ""
    echo "=============================================================================="
    echo "Git cache test completed at $(date -u +%Y-%m-%dT%H:%M:%SZ)"
    echo "=============================================================================="
