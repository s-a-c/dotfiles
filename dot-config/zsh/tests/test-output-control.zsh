...
test_conditional_output_sourced_filter() {
    ...
    if is_being_sourced; then
        ...
        [[ -z "$output" ]] || {
            echo "    ASSERTION FAILED: Sourced filter should not output when executed"
            return 1
        }
    fi
}
...
test_conditional_output_executed_filter() {
    ...
    if is_being_executed; then
        ...
        [[ -z "$output" ]] || {
            echo "    ASSERTION FAILED: Executed filter should not output when sourced"
            return 1
        }
    fi
}
...
test_context_echo_logging() {
    ...
    if [[ -f "$detection_log" ]]; then
        if grep -q "$test_message" "$detection_log" 2>/dev/null; then
            ...
            echo "    WARNING: Message not found in detection log (may be expected)"
            return 0  # Don't fail test, logging might be conditional
        fi
    else
        echo "    WARNING: Detection log file not found"
...
test_output_context_awareness() {
    ...
    [[ -n "$context" ]] || {
        echo "    ASSERTION FAILED: Context should not be empty"
        return 1
    }
...
run_all_tests() {
    echo "========================================================"
    echo "Output Control Test Suite"
    echo "========================================================"
    ...
    echo "=== Output Control Function Tests ==="
    run_test "Output Control Functions Exist" "test_output_control_functions_exist"
    run_test "Context Echo INFO Level" "test_context_echo_info_level"
    run_test "Context Echo WARN Level" "test_context_echo_warn_level"
    run_test "Context Echo ERROR Level" "test_context_echo_error_level"
    run_test "Context Echo Debug Mode" "test_context_echo_debug_mode"
    
    # Conditional Output Tests
    echo ""
    echo "=== Conditional Output Tests ==="
    ...
    # Logging Tests
    echo ""
    echo "=== Output Logging Tests ==="
    run_test "Context Echo Logging" "test_context_echo_logging"
    
    # Stream Routing Tests
    echo ""
    echo "=== Output Stream Tests ==="
    run_test "Output Stream Routing" "test_output_stream_routing"
    
    # Context Awareness Tests
    echo ""
    echo "=== Context Awareness Tests ==="
    ...
    if [[ $TEST_FAILED -eq 0 ]]; then
        echo ""
        echo "🎉 All output control tests passed!"
        return 0
    else
        echo ""
        echo "❌ $TEST_FAILED output control test(s) failed."
        return 1
    fi
}
...
if is_being_executed; then
    ...
    echo "Output control test functions loaded (sourced context)"
