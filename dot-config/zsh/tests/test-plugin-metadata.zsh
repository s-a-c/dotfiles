...
PLUGIN_METADATA_SCRIPT="${ZDOTDIR:-$HOME/.config/zsh}/.zshrc.d/20-plugins/01-plugin-metadata.zsh"

if [[ ! -f "$PLUGIN_METADATA_SCRIPT" ]]; then
    echo "ERROR: Plugin metadata script not found: $PLUGIN_METADATA_SCRIPT" >&2
    exit 1
fi
...
test_plugin_registration() {
    ...
    # Clear any existing registry for clean testing
    ...
    if register_plugin "test-plugin" "test/plugin" "oh-my-zsh" "" "" "Test plugin for testing"; then
        echo "    ✓ Basic plugin registration successful"
    else
        echo "    ✗ Basic plugin registration failed"
        return 1
    fi
    ...
    if register_plugin "dependent-plugin" "test/dependent" "oh-my-zsh" "test-plugin" "" "Plugin with dependencies"; then
        echo "    ✓ Plugin registration with dependencies successful"
    else
        echo "    ✗ Plugin registration with dependencies failed"
        return 1
    fi
    ...
    if register_plugin "conflicting-plugin" "test/conflict" "oh-my-zsh" "" "test-plugin" "Plugin with conflicts"; then
        echo "    ✓ Plugin registration with conflicts successful"
    else
        echo "    ✗ Plugin registration with conflicts failed"
        return 1
    fi
    
    # Verify plugins are registered
    if [[ -n "${ZSH_PLUGIN_REGISTRY[test-plugin]:-}" ]]; then
        echo "    ✓ Plugin metadata stored correctly"
    else
        echo "    ✗ Plugin metadata not stored"
        return 1
    fi
...
test_plugin_metadata_retrieval() {
    echo "    📊 Testing plugin metadata retrieval..."
    
    # Test retrieving existing plugin metadata
    local metadata=$(get_plugin_metadata "test-plugin")
    if [[ -n "$metadata" ]]; then
        echo "    ✓ Plugin metadata retrieval successful"
    else
        echo "    ✗ Plugin metadata retrieval failed"
        return 1
    fi
    ...
    if [[ -z "$missing_metadata" ]]; then
        echo "    ✓ Non-existent plugin correctly returns empty"
    else
        echo "    ✗ Non-existent plugin should return empty"
        return 1
    fi
    
    # Verify metadata contains expected fields
    if echo "$metadata" | grep -q '"name": "test-plugin"'; then
        echo "    ✓ Plugin metadata contains correct name"
    else
        echo "    ✗ Plugin metadata missing or incorrect name"
        return 1
    fi
...
test_dependency_resolution() {
    ...
    if [[ $dep_result -eq 0 ]]; then
        echo "    ✓ Dependencies correctly satisfied"
    else
        echo "    ✗ Dependencies should be satisfied but aren't"
        return 1
    fi
    ...
    if [[ $missing_dep_result -eq 2 ]]; then
        echo "    ✓ Missing dependencies correctly detected"
    else
        echo "    ✗ Missing dependencies not detected properly (result: $missing_dep_result)"
        return 1
    fi
...
test_conflict_detection() {
    ...
    if [[ $conflict_result -eq 2 ]]; then
        echo "    ✓ Plugin conflicts correctly detected"
    else
        echo "    ✗ Plugin conflicts not detected properly (result: $conflict_result)"
        return 1
    fi
    ...
    if [[ $no_conflict_result -eq 0 ]]; then
        echo "    ✓ No conflicts correctly identified"
    else
        echo "    ✗ False conflicts detected (result: $no_conflict_result)"
        return 1
    fi
...
test_load_order_resolution() {
    echo "    📊 Testing load order resolution..."
    
    # Test load order resolution with dependencies
    local load_order=$(resolve_plugin_load_order "dependent-plugin" "test-plugin")
    
    if echo "$load_order" | grep -q "test-plugin.*dependent-plugin"; then
        echo "    ✓ Load order correctly resolved with dependencies first"
    else
        echo "    ✗ Load order resolution failed"
        echo "    Expected: test-plugin before dependent-plugin"
        echo "    Got: $load_order"
        return 1
    fi
...
test_plugin_listing() {
    echo "    📊 Testing plugin listing functionality..."
    
    # Test simple listing
    local simple_list=$(list_plugins "simple")
    if echo "$simple_list" | grep -q "test-plugin"; then
        echo "    ✓ Simple plugin listing working"
    else
        echo "    ✗ Simple plugin listing failed"
        return 1
    fi
    
    # Test detailed listing
    local detailed_list=$(list_plugins "detailed")
    if echo "$detailed_list" | grep -q "Plugin: test-plugin"; then
        echo "    ✓ Detailed plugin listing working"
    else
        echo "    ✗ Detailed plugin listing failed"
        return 1
    fi
    
    # Test JSON listing
    local json_list=$(list_plugins "json")
    if echo "$json_list" | grep -q '"test-plugin":'; then
        echo "    ✓ JSON plugin listing working"
    else
        echo "    ✗ JSON plugin listing failed"
        return 1
    fi
...
test_plugin_configuration() {
    echo "    📊 Testing plugin configuration options..."
    ...
    if [[ $strict_result -eq 1 ]]; then
        echo "    ✓ Strict dependency mode working"
    else
        echo "    ✗ Strict dependency mode not working (result: $strict_result)"
    ...
    if [[ $error_result -eq 1 ]]; then
        echo "    ✓ Error conflict resolution mode working"
    else
        echo "    ✗ Error conflict resolution mode not working (result: $error_result)"
...
test_plugin_framework_integration() {
    echo "    📊 Testing plugin framework integration..."
    ...
    if [[ -d "$ZSH_PLUGIN_REGISTRY_DIR" ]]; then
        echo "    ✓ Plugin registry directory created"
    else
        integration_issues=$((integration_issues + 1))
        echo "    ✗ Plugin registry directory not created"
    fi
    
    # Check if metadata file exists
    if [[ -f "$ZSH_PLUGIN_METADATA_FILE" ]]; then
        echo "    ✓ Plugin metadata file exists"
    else
        integration_issues=$((integration_issues + 1))
        echo "    ✗ Plugin metadata file not created"
    fi
    
    # Check if dependency cache exists
    if [[ -f "$ZSH_PLUGIN_DEPENDENCY_CACHE" ]]; then
        echo "    ✓ Plugin dependency cache exists"
    else
        integration_issues=$((integration_issues + 1))
        echo "    ✗ Plugin dependency cache not created"
    fi
    ...
    if [[ $integration_issues -eq 0 ]]; then
        echo "    ✓ Plugin framework integration successful"
        return 0
    else
        echo "    ✗ Plugin framework integration has $integration_issues issues"
        return 1
    fi
}
...
run_all_tests() {
    echo "========================================================"
    echo "Plugin Metadata Framework Test Suite"
    echo "========================================================"
    echo "Timestamp: $(date -u '+%Y-%m-%d %H:%M:%S UTC')"
    echo "Execution Context: $(get_execution_context)"
    echo "Plugin Metadata Version: ${ZSH_PLUGIN_METADATA_VERSION:-unknown}"
    ...
    echo "=== Plugin Metadata Function Tests ==="
    ...
    echo ""
    echo "=== Dependency and Conflict Tests ==="
    ...
    if [[ $TEST_FAILED -eq 0 ]]; then
        echo ""
        echo "🎉 All plugin metadata framework tests passed!"
        return 0
    else
        echo ""
        echo "❌ $TEST_FAILED plugin metadata framework test(s) failed."
        return 1
    fi
}
...
if is_being_executed; then
    ...
    echo "Plugin metadata framework test functions loaded (sourced context)"
