...
test_context_log_functionality() {
    ...
    if [[ -f "$ctx_log_file" ]]; then
        ...
        echo "    WARNING: Context log file does not exist, skipping log verification"
        return 0
    fi
}
...
test_current_execution_context() {
    ...
    echo "  Current execution context: $context"
...
run_all_tests() {
    echo "========================================================"
    echo "Source/Execute Detection System Test Suite"
    echo "========================================================"
    ...
    echo ""
    echo "=== Output Control Tests ==="
    run_test "Output Functions Exist" "test_output_functions_exist"
    run_test "Context Echo Levels" "test_context_echo_levels"
    run_test "Context Echo Force Output" "test_context_echo_force_output"
...
# Main execution logic using the detection system we're testing
main() {
...
if is_being_sourced; then
    echo "Source/Execute Detection test functions loaded (sourced context)"
    echo "Available test functions:"
    echo "  - run_all_tests"
    echo "  - Individual test functions (test_*)"
    echo "  - Test framework functions (run_test, assert_*)"
fi
