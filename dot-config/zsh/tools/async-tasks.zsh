...
# ---------------------------------------------------------------------------
# %N gives current sourced file (zsh feature). Fallback to $0 if unset.
__ASYNC_TASKS_SRC="${(%):-%N}"
if [[ "$__ASYNC_TASKS_SRC" == "async-tasks.zsh" || "$__ASYNC_TASKS_SRC" == "" ]]; then
  # Fallback heuristic: search relative tools directory
  for __p in "${0:A:h}" "${PWD}/tools" "${PWD}"; do
    [[ -r "${__p}/async-tasks.zsh" ]] && __ASYNC_TASKS_SRC="${__p}/async-tasks.zsh" && break
  done
fi
