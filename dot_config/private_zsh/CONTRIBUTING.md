Compliant with /Users/s-a-c/.config/ai/guidelines.md v<checksum>
...
Quick start (stow)
------------------
From the repository root (~/.config):

  # Create the runtime symlink: ~/.config/zsh -> ~/.config/zsh
...
- Suggested subject for these housekeeping changes:
  ...
  - Add `zsh/README-ZDOTDIR.md` documenting canonical repo (`~/.config/zsh`) → runtime (`~/.config/zsh`) layout
