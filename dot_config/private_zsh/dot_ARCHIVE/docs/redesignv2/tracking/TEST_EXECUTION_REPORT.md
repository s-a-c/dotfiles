...
Compliant with [/Users/s-a-c/.config/ai/guidelines.md](/Users/s-a-c/.config/ai/guidelines.md) vb7f03a299a01b1b6d7c8be5a74646f0b5127cbc5b5d614c8b4c20fc99bc21620
...
3. **Test Suite Alternative**
   ...
   for test in ~/.config/zsh/tests/unit/core/*.zsh; do
     echo "Testing: $test"
     zsh -f "$test"
   done
   ```
...
zsh -f ~/.config/zsh/tests/unit/core/test-core-functions-manifest.zsh

# Migration (working with bash)
/bin/bash ~/.config/zsh/tools/migrate-to-redesign.sh --dry-run --zshenv /tmp/test.zshenv
MIGRATE_FORCE=1 /bin/bash ~/.config/zsh/tools/migrate-to-redesign.sh --apply --zshenv /tmp/test.zshenv
```
