...
```sh
ZDOTDIR="$HOME/.config/zsh" ZSH_FORCE_ZGEN_RESET=1 zsh -i
```
...
grep -i "parameter not set\|nounset" "$HOME/.config/zsh/logs"/* 2>/dev/null || true
```
